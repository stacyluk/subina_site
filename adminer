...
$sc = !preg_match('~^(unsafe_raw)?$~', ini_get("filter.default"));
if ($sc || ini_get("filter.default_flags")) {
...
idf_unescape(
    ...
    $qd = substr($w, -1);
    return
        str_replace($qd.$qd, $qd, substr($w, 1, -1));
}
    ...
    if (get_magic_quotes_gpc()) {
        while (list($_, $X) = each($ef)) {
            foreach ($X
                ...
                unset($ef[$_][$id]);
                if (is_array($W)) {
                    $ef[$_][stripslashes($id)] = $W;
                    $ef[] =& $ef[$_][stripslashes($id)];
                } else {
                    $ef[$_][stripslashes($id)] = ($sc ? $W : stripslashes($W));
                }
            }
        }
    }
...
bracket_escape(
    ...
    static $Mg = array(':' => ':1', ']' => ':2', '[' => ':3', '"' => ':4');
    return
        strtr($w, ($Aa ? array_flip($Mg) : $Mg));
}
...
min_version(
    ...
    $Mf = $g->server_info;
    if ($Cd && preg_match('~([\d.]+)-MariaDB~', $Mf, $D)) {
        $Mf = $D[1];
        $oh = $Cd;
    }
    return (version_compare($Mf, $oh) >= 0);
}
...
checkbox(
    ...
    $K = "<input type='checkbox' name='$F' value='".h($Y)."'".($Oa ? " checked" : "").($nd ? " aria-labelledby='$nd'" : "").">".($ne ? script("qsl('input').onclick = function () { $ne };",
            "") : "");
    return ($md != "" || $Sa ? "<label".($Sa ? " class='$Sa'" : "").">$K".h($md)."</label>" : $K);
}
...
optionlist(
    ...
    foreach ($re
        ...
        $se = array($id => $W);
        if (is_array($W)) {
            $K .= '<optgroup label="'.h($id).'">';
            $se = $W;
        }
        foreach ($se
                 as $_ => $X) {
            $K .= '<option'.($ih || is_string($_) ? ' value="'.h($_).'"' : '').(($ih || is_string($_) ? (string) $_ : $X) === $Hf ? ' selected' : '').'>'.h($X);
        }
        if (is_array($W)) {
            $K .= '</optgroup>';
        }
...
html_select(
    ...
    if ($me) {
        return "<select name='".h($F)."'".($nd ? " aria-labelledby='$nd'" : "").">".optionlist($re,
...
select_input(
    ...
    $ug = ($re ? "select" : "input");
    return "<$ug$xa".($re ? "><option value=''>$Re".optionlist($re, $Y,
                true)."</select>" : " size='10' value='".h($Y)."' placeholder='$Re'>").($me ? script("qsl('$ug').onchange = $me;",
            "") : "");
}
...
print_fieldset(
    ...
    echo "<fieldset><legend>", "<a href='#fieldset-$v'>$vd</a>", script("qsl('a').onclick = partial(toggle, 'fieldset-$v');",
        ""), "</legend>", "<div id='fieldset-$v'".($rh ? "" : " class='hidden'").">\n";
}
...
set_password(
    ...
    $_SESSION["pwds"][$nh][$O][$V] = ($_COOKIE["adminer_key"] && is_string($Ne) ? array(
        encrypt_string($Ne, $_COOKIE["adminer_key"])
    ) : $Ne);
}
...
where(
    ...
    foreach ((array) $Z["where"] as $_ => $X) {
        ...
        $c = escape_key($_);
        $K[] = $c.($z == "sql" && is_numeric($X) && preg_match('~\.~',
                $X) ? " LIKE ".q($X) : ($z == "mssql" ? " LIKE ".q(preg_replace('~[_%[]~', '[\0]',
                    $X)) : " = ".unconvert_field($n[$_], q($X))));
...
where_link(
    ...
    return "&where%5B$u%5D%5Bcol%5D=".urlencode($c)."&where%5B$u%5D%5Bop%5D=".urlencode(($Y !== null ? $oe : "IS NULL"))."&where%5B$u%5D%5Bval%5D=".urlencode($Y);
}
...
convert_fields(
    ...
    foreach ($d
        ...
        $va = convert_field($n[$_]);
        if ($va) {
            $K .= ", $va AS ".idf_escape($_);
        }
    }
    return $K;
}
...
cookie(
    ...
    global $ba;
    return
        header("Set-Cookie: $F=".urlencode($Y).($yd ? "; expires=".gmdate("D, d M Y H:i:s",
...
stop_session(
    ...
    $hh = ini_bool("session.use_cookies");
    if (!$hh || $vc) {
        session_write_close();
        if ($hh && @ini_set("session.use_cookies", false) === false) {
            session_start();
        }
    }
}
...
auth_url(
    ...
    global $Hb;
    preg_match('~([^?]*)\??(.*)~',
        remove_from_uri(implode("|", array_keys($Hb))."|username|".($j !== null ? "db|" : "").session_name()), $D);
    return "$D[1]?".(sid() ? SID."&" : "").($nh != "server" || $O != "" ? urlencode($nh)."=".urlencode($O)."&" : "")."username=".urlencode($V).($j != "" ? "&db=".urlencode($j) : "").($D[2] ? "&$D[2]" : "");
}
...
queries_redirect(
    ...
    return
        query_redirect($hf, $C, $E, $mf, false, !$mf, $Ag);
}
...
remove_from_uri(
    ...
    return
        substr(preg_replace("~(?<=[?&])($Fe".(SID ? "" : "|".session_name()).")=[^&]*&~", '', "$_SERVER[REQUEST_URI]&"),
            0, -1);
}
...
upload_error(
    ...
    $Id = ($l == UPLOAD_ERR_INI_SIZE ? ini_get("upload_max_filesize") : 0);
    return ($l ? lang(2).($Id ? " ".lang(3, $Id) : "") : lang(4));
}
...
repeat_pattern(
    ...
    return
        str_repeat("$Pe{0,65535}", $wd / 65535)."$Pe{0,".($wd % 65535)."}";
}
...
enum_input(
    ...
    global $b;
    preg_match_all("~'((?:[^']|'')*)'~", $m["length"], $Dd);
    $K = ($Vb !== null ? "<label><input type='$U'$xa value='$Vb'".((is_array($Y) ? in_array($Vb,
            $Y) : $Y === 0) ? " checked" : "")."><i>".lang(7)."</i></label>" : "");
    foreach ($Dd[1] as $u => $X) {
        $X = stripcslashes(str_replace("''", "'", $X));
        $Oa = (is_int($Y) ? $Y == $u + 1 : (is_array($Y) ? in_array($u + 1, $Y) : $Y === $X));
        $K .= " <label><input type='$U'$xa value='".($u + 1)."'".($Oa ? ' checked' : '').'>'.h($b->editVal($X,
                $m)).'</label>';
    }
    return $K;
}
...
input(
    ...
    $uf = ($z == "mssql" && $m["auto_increment"]);
    if ($uf && !$_POST["save"]) {
        $r = null;
    }
    $Bc = (isset($_GET["select"]) || $uf ? array("orig" => lang(8)) : array()) + $b->editFunctions($m);
    $xa = " name='fields[$F]'";
    if ($m["type"] == "enum") {
        echo
            h($Bc[""])."<td>".$b->editInput($_GET["edit"], $m, $xa, $Y);
    } else {
        $Jc = (in_array($r, $Bc) || isset($Bc[$r]));
        echo (count($Bc) > 1 ? "<select name='function[$F]'>".optionlist($Bc,
                    $r === null || $Jc ? $r : "")."</select>".on_help("getTarget(event).value.replace(/^SQL\$/, '')",
                    1).script("qsl('select').onchange = functionChange;", "") : h(reset($Bc))).'<td>';
        $Yc = $b->editInput($_GET["edit"], $m, $xa, $Y);
        if ($Yc != "") {
            echo $Yc;
        } elseif (preg_match('~bool~', $m["type"])) {
            echo "<input type='hidden'$xa value='0'>"."<input type='checkbox'".(preg_match('~^(1|t|true|y|yes|on)$~i',
                    $Y) ? " checked='checked'" : "")."$xa value='1'>";
        } elseif ($m["type"] == "set") {
            preg_match_all("~'((?:[^']|'')*)'~", $m["length"], $Dd);
            foreach ($Dd[1] as $u => $X) {
                $X = stripcslashes(str_replace("''", "'", $X));
                $Oa = (is_int($Y) ? ($Y >> $u) & 1 : in_array($X, explode(",", $Y), true));
                echo " <label><input type='checkbox' name='fields[$F][$u]' value='".(1 << $u)."'".($Oa ? ' checked' : '').">".h($b->editVal($X,
                        $m)).'</label>';
            }
        } elseif (preg_match('~blob|bytea|raw|file~', $m["type"]) && ini_bool("file_uploads")) {
            echo "<input type='file' name='fields-$F'>";
        } elseif (($zg = preg_match('~text|lob|memo~i', $m["type"])) || preg_match("~\n~", $Y)) {
            if ($zg && $z != "sqlite") {
                $xa .= " cols='50' rows='12'";
            } else {
            ...
            $Kd = (!preg_match('~int~', $m["type"]) && preg_match('~^(\d+)(,(\d+))?$~', $m["length"],
                $D) ? ((preg_match("~binary~",
                    $m["type"]) ? 2 : 1) * $D[1] + ($D[3] ? 1 : 0) + ($D[2] && !$m["unsigned"] ? 1 : 0)) : ($Ug[$m["type"]] ? $Ug[$m["type"]] + ($m["unsigned"] ? 0 : 1) : 0));
            if ($z == 'sql' && min_version(5.6) && preg_match('~time~', $m["type"])) {
                $Kd += 7;
            }
            echo "<input".((!$Jc || $r === "") && preg_match('~(?<!o)int(?!er)~', $m["type"]) && !preg_match('~\[\]~',
                    $m["full_type"]) ? " type='number'" : "")." value='".h($Y)."'".($Kd ? " data-maxlength='$Kd'" : "").(preg_match('~char|binary~',
                    $m["type"]) && $Kd > 20 ? " size='40'" : "")."$xa>";
        }
...
fields_from_edit()
{
    ...
    foreach ((array) $_POST["fields"] as $_ => $X) {
        ...
        $K[$F] = array(
            "field" => $F, "privileges" => array("insert" => 1, "update" => 1), "null" => 1,
            "auto_increment" => ($_ == $k->primary),
        );
    }
    return $K;
}
...
search_tables()
{
    ...
    foreach (table_status('', true) as $Q => $R) {
        ...
        if (isset($R["Engine"]) && $F != "" && (!$_POST["tables"] || in_array($Q, $_POST["tables"]))) {
            $J = $f->query("SELECT".limit("1 FROM ".table($Q),
...
dump_headers(
    ...
    global $b;
    $K = $b->dumpHeaders($Rc, $Rd);
    $Ce = $_POST["output"];
    if ($Ce != "text") {
        header("Content-Disposition: attachment; filename=".$b->dumpFilename($Rc).".$K".($Ce != "file" && !preg_match('~[^0-9a-z]~',
                $Ce) ? ".$Ce" : ""));
    }
    session_write_close();
    ob_flush();
...
select_value(
    ...
    if (is_array($X)) {
        ...
        foreach ($X
                 as $id => $W) {
            $K .= "<tr>".($X != array_values($X) ? "<th>".h($id) : "")."<td>".select_value($W, $B, $m, $_g);
        }
...
is_mail(
    ...
    $Pe = "$wa+(\\.$wa+)*@($Gb?\\.)+$Gb";
    return
        is_string($Sb) && preg_match("(^$Pe(,\\s*$Pe)*\$)i", $Sb);
}
...
count_rows(
    ...
    return ($ed && ($z == "sql" || count($t) == 1) ? "SELECT COUNT(DISTINCT ".implode(", ",
            $t).")$I" : "SELECT COUNT(*)".($ed ? " FROM (SELECT 1$I GROUP BY ".implode(", ", $t).") x" : $I));
}
...
edit_form(
    ...
    page_header(($ch ? lang(10) : lang(11)), $l, array("select" => array($a, $og)), $og);
    if ($L === false) {
...
';
    if (!$n) {
        ...
        foreach ($n
            ...
            $Y = ($L !== null ? ($L[$F] != "" && $z == "sql" && preg_match("~enum|set~",
                $m["type"]) ? (is_array($L[$F]) ? array_sum($L[$F]) : +$L[$F]) : $L[$F]) : (!$ch && $m["auto_increment"] ? "" : (isset($_GET["select"]) ? false : $yb)));
            if (!$_POST["save"] && is_string($Y)) {
                $Y = $b->editVal($Y, $m);
            }
            $r = ($_POST["save"] ? (string) $_POST["function"][$F] : ($ch && preg_match('~^CURRENT_TIMESTAMP~i',
                $m["on_update"]) ? "now" : ($Y === false ? null : ($Y !== null ? '' : 'NULL'))));
...
global $b, $f, $k, $Hb, $Ob, $Yb, $l, $Bc, $Fc, $ba, $Xc, $z, $ca, $pd, $le, $Qe, $gg, $Kc, $T, $Og, $Ug, $bh, $ga;
...
remove_slashes(array(&$_GET, &$_POST, &$_COOKIE), $sc);
...
$pd = array(
    'en' => 'English', 'ar' => 'العربية', 'bg' => 'Български', 'bn' => 'বাংলা', 'bs' => 'Bosanski', 'ca' => 'Català',
    'cs' => 'Čeština', 'da' => 'Dansk', 'de' => 'Deutsch', 'el' => 'Ελληνικά', 'es' => 'Español', 'et' => 'Eesti',
    'fa' => 'فارسی', 'fi' => 'Suomi', 'fr' => 'Français', 'gl' => 'Galego', 'he' => 'עברית', 'hu' => 'Magyar',
    'id' => 'Bahasa Indonesia', 'it' => 'Italiano', 'ja' => '日本語', 'ka' => 'ქართული', 'ko' => '한국어', 'lt' => 'Lietuvių',
    'ms' => 'Bahasa Melayu', 'nl' => 'Nederlands', 'no' => 'Norsk', 'pl' => 'Polski', 'pt' => 'Português',
    'pt-br' => 'Português (Brazil)', 'ro' => 'Limba Română', 'ru' => 'Русский', 'sk' => 'Slovenčina',
    'sl' => 'Slovenski', 'sr' => 'Српски', 'ta' => 'த‌மிழ்', 'th' => 'ภาษาไทย', 'tr' => 'Türkçe', 'uk' => 'Українська',
...
lang(
    ...
    $Ng = ($Og[$w] ? $Og[$w] : $w);
    if (is_array($Ng)) {
        $Te = ($ce == 1 ? 0 : ($ca == 'cs' || $ca == 'sk' ? ($ce && $ce < 5 ? 1 : 2) : ($ca == 'fr' ? (!$ce ? 0 : 1) : ($ca == 'pl' ? ($ce % 10 > 1 && $ce % 10 < 5 && $ce / 10 % 10 != 1 ? 1 : 2) : ($ca == 'sl' ? ($ce % 100 == 1 ? 0 : ($ce % 100 == 2 ? 1 : ($ce % 100 == 3 || $ce % 100 == 4 ? 2 : 3))) : ($ca == 'lt' ? ($ce % 10 == 1 && $ce % 100 != 11 ? 0 : ($ce % 10 > 1 && $ce / 10 % 10 != 1 ? 1 : 2)) : ($ca == 'bs' || $ca == 'ru' || $ca == 'sr' || $ca == 'uk' ? ($ce % 10 == 1 && $ce % 100 != 11 ? 0 : ($ce % 10 > 1 && $ce % 10 < 5 && $ce / 10 % 10 != 1 ? 1 : 2)) : 1)))))));
        $Ng = $Ng[$Te];
    }
    $ua = func_get_args();
    array_shift($ua);
    $yc = str_replace("%d", "%s", $Ng);
    if ($yc != $Ng) {
...
{
    global $ca, $pd;
    echo "<form action='' method='post'>\n<div id='lang'>", lang(19).": ".html_select("lang", $pd, $ca,
            "this.form.submit();"), " <input type='submit' value='".lang(20)."' class='hidden'>\n", "<input type='hidden' name='token' value='".get_token()."'>\n";
    echo "</div>\n</form>\n";
}
...
if (isset($pd[$_COOKIE["adminer_lang"]])) {
    ...
    $la = array();
    preg_match_all('~([-a-z]+)(;q=([0-9.]+))?~', str_replace("_", "-", strtolower($_SERVER["HTTP_ACCEPT_LANGUAGE"])),
        $Dd, PREG_SET_ORDER);
    foreach ($Dd
             as $D) {
        $la[$D[1]] = (isset($D[3]) ? $D[3] : 1);
    }
    arsort($la);
    foreach ($la
             as $_ => $H) {
...
if ($_SESSION["translations_version"] != 2463266818) {
    $Og = array();
    $_SESSION["translations_version"] = 2463266818;
}
function
get_translations(
    $od
...
Min_SQL
{
    ...
    select(
        ...
        global $b, $z;
        $ed = (count($t) < count($N));
        $I = $b->selectQueryBuild($N, $Z, $t, $te, $A, $G);
        if (!$I) {
            $I = "SELECT".limit(($_GET["page"] != "last" && $A != "" && $t && $ed && $z == "sql" ? "SQL_CALC_FOUND_ROWS " : "").implode(", ",
                        $N)."\nFROM ".table($Q),
                    ($Z ? "\nWHERE ".implode(" AND ", $Z) : "").($t && $ed ? "\nGROUP BY ".implode(", ",
                            $t) : "").($te ? "\nORDER BY ".implode(", ", $te) : ""), ($A != "" ? +$A : null),
                    ($G ? $A * $G : 0), "\n");
        }
        $bg = microtime(true);
    ...
    update(
        ...
        $I = table($Q)." SET$Lf".implode(",$Lf", $lh);
        return
    ...
    value(
        ...
        return (method_exists($this->_conn, 'value') ? $this->_conn->value($X,
            $m) : (is_resource($X) ? stream_get_contents($X) : $X));
    }
...
if (!defined("DRIVER")) {
    ...
    if (extension_loaded("mysqli")) {
            ...
            MySQLi
        {
            ...
            connect(
                ...
                list($Pc, $Se) = explode(":", $O, 2);
                $ag = $b->connectSsl();
                if ($ag) {
                    $this->ssl_set($ag['key'], $ag['cert'], $ag['ca'], '', '');
                }
                $K = @$this->real_connect(($O != "" ? $Pc : ini_get("mysqli.default_host")),
                    ($O.$V != "" ? $V : ini_get("mysqli.default_user")),
                    ($O.$V.$Ne != "" ? $Ne : ini_get("mysqli.default_pw")), $tb,
                    (is_numeric($Se) ? $Se : ini_get("mysqli.default_port")), (!is_numeric($Se) ? $Se : $Uf),
                    ($ag ? 64 : 0));
        ...
        Min_DB
        {
            ...
            connect(
                ...
                $this->_link = @mysql_connect(($O != "" ? $O : ini_get("mysql.default_host")),
                    ...
                    ("$O$V$Ne" != "" ? $Ne : ini_get("mysql.default_password")), true, 131072);
                if ($this->_link) {
            ...
            Min_PDO
        {
            ...
            connect(
                ...
                if ($ag) {
                    if (!empty($ag['key'])) {
                        $re[PDO::MYSQL_ATTR_SSL_KEY] = $ag['key'];
                    }
                    if (!empty($ag['cert'])) {
                        $re[PDO::MYSQL_ATTR_SSL_CERT] = $ag['cert'];
                    }
                    if (!empty($ag['ca'])) {
                        $re[PDO::MYSQL_ATTR_SSL_CA] = $ag['ca'];
                    }
                }
                $this->dsn("mysql:charset=utf8;host=".str_replace(":", ";unix_socket=",
                        preg_replace('~:(\d)~', ';port=\1', $O)), $V, $Ne, $re);
                return
                    true;
            }
        ...
        Min_SQL
    {
        ...
        insertUpdate(
            ...
            foreach ($M
                     as $P) {
                $Y = "(".implode(", ", $P).")";
                if ($lh && (strlen($Xe) + $wd + strlen($Y) + strlen($jg) > 1e6)) {
        ...
        convertSearch(
            ...
            return (preg_match('~char|text|enum|set~', $m["type"]) && !preg_match("~^utf8~",
                $m["collation"]) && preg_match('~[\x80-\xFF]~',
                $X['val']) ? "CONVERT($w USING ".charset($this->_conn).")" : $w);
        }
        ...
        tableHelp(
            ...
            if (information_schema(DB)) {
                return
                    strtolower(($Bd ? "information-schema-$F-table/" : str_replace("_", "-", $F)."-table.html"));
            }
            if (DB == "mysql") {
                return ($Bd ? "mysql$F-table/" : "system-database.html");
            }
        }
    }
    ...
    db_collation(
        ...
        if (preg_match('~ COLLATE ([^ ]+)~', $h, $D)) {
            $K = $D[1];
        } elseif (preg_match('~ CHARACTER SET ([^ ]+)~', $h, $D)) {
            $K = $Ya[$D[1]][-1];
        }
        return $K;
    }
    ...
    table_status(
        ...
        $K = array();
        foreach (get_rows($nc && min_version(5) ? "SELECT TABLE_NAME AS Name, ENGINE AS Engine, TABLE_COMMENT AS Comment FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() ".($F != "" ? "AND TABLE_NAME = ".q($F) : "ORDER BY Name") : "SHOW TABLE STATUS".($F != "" ? " LIKE ".q(addcslashes($F,
                    "%_\\")) : "")) as $L) {
            if ($L["Engine"] == "InnoDB") {
    ...
    fields(
        ...
        foreach (get_rows("SHOW FULL COLUMNS FROM ".table($Q)) as $L) {
            ...
            $K[$L["Field"]] = array(
                "field" => $L["Field"], "full_type" => $L["Type"], "type" => $D[1], "length" => $D[2],
                "unsigned" => ltrim($D[3].$D[4]),
                "default" => ($L["Default"] != "" || preg_match("~char|set~", $D[1]) ? $L["Default"] : null),
                "null" => ($L["Null"] == "YES"), "auto_increment" => ($L["Extra"] == "auto_increment"),
                "on_update" => (preg_match('~^on update (.+)~i', $L["Extra"], $D) ? $D[1] : ""),
                "collation" => $L["Collation"], "privileges" => array_flip(preg_split('~, *~', $L["Privileges"])),
                "comment" => $L["Comment"], "primary" => ($L["Key"] == "PRI"),
    ...
    foreign_keys(
        ...
        if ($kb) {
            preg_match_all("~CONSTRAINT ($Pe) FOREIGN KEY ?\\(((?:$Pe,? ?)+)\\) REFERENCES ($Pe)(?:\\.($Pe))? \\(((?:$Pe,? ?)+)\\)(?: ON DELETE ($le))?(?: ON UPDATE ($le))?~",
                $kb, $Dd, PREG_SET_ORDER);
            foreach ($Dd
                     as $D) {
                preg_match_all("~$Pe~", $D[2], $Vf);
                preg_match_all("~$Pe~", $D[5], $vg);
                $K[idf_unescape($D[1])] = array(
                    "db" => idf_unescape($D[4] != "" ? $D[3] : $D[4]),
                    "table" => idf_unescape($D[4] != "" ? $D[4] : $D[3]), "source" => array_map('idf_unescape', $Vf[0]),
                    "target" => array_map('idf_unescape', $vg[0]), "on_delete" => ($D[6] ? $D[6] : "RESTRICT"),
        ...
        return
            array(
                "select" => preg_replace('~^(?:[^`]|`[^`]*`)*\s+AS\s+~isU', '',
                    $f->result("SHOW CREATE VIEW ".table($F), 1))
            );
    }
    ...
    alter_table(
        ...
        $cg = ($cb !== null ? " COMMENT=".q($cb) : "").($Wb ? " ENGINE=".q($Wb) : "").($Xa ? " COLLATE ".q($Xa) : "").($za != "" ? " AUTO_INCREMENT=$za" : "");
        if ($Q == "") {
            return
                queries("CREATE TABLE ".table($F)." (\n".implode(",\n", $ta)."\n)$cg$Ke");
        }
        if ($Q != $F) {
            $ta[] = "RENAME TO ".table($F);
        }
        if ($cg) {
            $ta[] = ltrim($cg);
        }
    ...
    drop_views(
        ...
        return
            queries("DROP VIEW ".implode(", ", array_map('table', $qh)));
    }
    ...
    copy_tables(
        ...
        foreach ($qh
            ...
            $F = ($vg == DB ? table("copy_$Q") : idf_escape($vg).".".table($Q));
            $ph = view($Q);
            if (($_POST["overwrite"] && !queries("DROP VIEW IF EXISTS $F")) || !queries("CREATE VIEW $F AS $ph[select]")) {
                return
                    false;
            }
        }
    ...
    trigger_options()
    {
        return
            array(
                "Timing" => array("BEFORE", "AFTER"), "Event" => array("INSERT", "UPDATE", "DELETE"),
    ...
    routine(
        ...
        global $f, $Yb, $Xc, $Ug;
        $ra = array(
            "bool", "boolean", "integer", "double precision", "real", "dec", "numeric", "fixed", "national char",
            "national varchar"
        );
        $Wf = "(?:\\s|/\\*[\s\S]*?\\*/|(?:#|-- )[^\n]*\n?|--\r?\n)";
        $Tg = "((".implode("|", array_merge(array_keys($Ug),
                $ra)).")\\b(?:\\s*\\(((?:[^'\")]|$Yb)++)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s,]+)['\"]?)?";
        $Pe = "$Wf*(".($U == "FUNCTION" ? "" : $Xc).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$Tg";
        $h = $f->result("SHOW CREATE $U ".idf_escape($F), 2);
        preg_match("~\\(((?:$Pe\\s*,?)*)\\)\\s*".($U == "FUNCTION" ? "RETURNS\\s+$Tg\\s+" : "")."(.*)~is", $h, $D);
        $n = array();
        preg_match_all("~$Pe\\s*,?~is", $D[1], $Dd, PREG_SET_ORDER);
        foreach ($Dd
                 as $Fe) {
            $F = str_replace("``", "`", $Fe[2]).$Fe[3];
            $n[] = array(
                "field" => $F, "type" => strtolower($Fe[5]),
                "length" => preg_replace_callback("~$Yb~s", 'normalize_enum', $Fe[6]),
                "unsigned" => strtolower(preg_replace('~\s+~', ' ', trim("$Fe[8] $Fe[7]"))), "null" => 1,
                "full_type" => $Fe[4], "inout" => strtoupper($Fe[1]), "collation" => strtolower($Fe[9]),
            );
        }
        if ($U != "FUNCTION") {
            return
                array("fields" => $n, "definition" => $D[11]);
        }
        return
            array(
                "fields" => $n,
                "returns" => array("type" => $D[12], "length" => $D[13], "unsigned" => $D[15], "collation" => $D[16]),
                "definition" => $D[17], "language" => "SQL",
            );
    }
    ...
    foreach (array(
                 lang(24) => array(
                     "tinyint" => 3, "smallint" => 5, "mediumint" => 8, "int" => 10, "bigint" => 20, "decimal" => 66,
                     "float" => 12, "double" => 21
                 ), lang(25) => array("date" => 10, "datetime" => 19, "timestamp" => 19, "time" => 10, "year" => 4),
                 lang(23) => array(
                     "char" => 255, "varchar" => 65535, "tinytext" => 255, "text" => 65535, "mediumtext" => 16777215,
                     "longtext" => 4294967295
                 ), lang(26) => array("enum" => 65535, "set" => 64), lang(27) => array(
            "bit" => 20, "binary" => 255, "varbinary" => 65535, "tinyblob" => 255, "blob" => 65535,
            "mediumblob" => 16777215, "longblob" => 4294967295
        ), lang(28) => array(
            "geometry" => 0, "point" => 0, "linestring" => 0, "polygon" => 0, "multipoint" => 0, "multilinestring" => 0,
            "multipolygon" => 0, "geometrycollection" => 0
        ),
             ) as $_ => $X) {
        $Ug += $X;
        $gg[$_] = array_keys($X);
    }
    $bh = array("unsigned", "zerofill", "unsigned zerofill");
    $pe = array(
        "=", "<", ">", "<=", ">=", "!=", "LIKE", "LIKE %%", "REGEXP", "IN", "FIND_IN_SET", "IS NULL", "NOT LIKE",
        "NOT REGEXP", "NOT IN", "IS NOT NULL", "SQL"
    );
    $Bc = array(
        "char_length", "date", "from_unixtime", "lower", "round", "floor", "ceil", "sec_to_time", "time_to_sec", "upper"
    );
    $Fc = array("avg", "count", "count distinct", "group_concat", "max", "min", "sum");
    $Ob = array(
        array("char" => "md5/sha1/password/encrypt/uuid", "binary" => "md5/sha1", "date|time" => "now",), array(
            number_type() => "+/-", "date" => "+ interval/- interval", "time" => "addtime/subtime",
            "char|text" => "concat",
        )
    );
}
...
Adminer
{
    ...
    {
        global $Hb;
        echo "<table cellspacing='0' class='layout'>\n", $this->loginFormField('driver', '<tr><th>'.lang(29).'<td>',
            html_select("auth[driver]", $Hb, DRIVER, "loginDriver(this);")."\n"), $this->loginFormField('server',
            '<tr><th>'.lang(30).'<td>',
            ...
            '<input name="auth[db]" value="'.h($_GET["db"]).'" autocapitalize="off">'."\n"), "</table>\n", "<p><input type='submit' value='".lang(34)."'>\n", checkbox("auth[permanent]",
                1, $_COOKIE["adminer_permanent"], lang(35))."\n";
    }
    ...
    selectQuery(
        ...
        if (!$mc && ($th = $k->warnings())) {
            $v = "warnings";
            $K = ", <a href='#$v'>".lang(42)."</a>".script("qsl('a').onclick = partial(toggle, '$v');",
                    "")."$K<div id='$v' class='hidden'>\n$th</div>\n";
        }
    ...
    selectVal(
        ...
        $K = ($X === null ? "<i>NULL</i>" : (preg_match("~char|binary|boolean~", $m["type"]) && !preg_match("~var~",
            $m["type"]) ? "<code>$X</code>" : $X));
    ...
    selectColumnsPrint(
        ...
        foreach ($N
                 as $_ => $X) {
            $X = $_GET["columns"][$_];
            $c = select_input(" name='columns[$u][col]'", $d, $X["col"],
                ($_ !== "" ? "selectFieldChange" : "selectAddRow"));
            echo "<div>".($Bc || $Fc ? "<select name='columns[$u][fun]'>".optionlist(array(-1 => "") + array_filter(array(
                            lang(50) => $Bc, lang(51) => $Fc
                        )),
                        $X["fun"])."</select>".on_help("getTarget(event).value && getTarget(event).value.replace(/ |\$/, '(') + ')'",
                        1).script("qsl('select').onchange = function () { helpClose();".($_ !== "" ? "" : " qsl('select, input', this.parentNode).onchange();")." };",
                        "")."($c)" : $c)."</div>\n";
            $u++;
        }
        echo "</div></fieldset>\n";
    ...
    selectSearchPrint(
        ...
        foreach ($y
            ...
            if ($x["type"] == "FULLTEXT") {
                echo "<div>(<i>".implode("</i>, <i>", array_map('h',
                        $x["columns"]))."</i>) AGAINST", " <input type='search' name='fulltext[$u]' value='".h($_GET["fulltext"][$u])."'>", script("qsl('input').oninput = selectFieldChange;",
                    ""), checkbox("boolean[$u]", 1, isset($_GET["boolean"][$u]), "BOOL"), "</div>\n";
            }
        }
        ...
        foreach (array_merge((array) $_GET["where"], array(array())) as $u => $X) {
            if (!$X || ("$X[col]$X[val]" != "" && in_array($X["op"], $this->operators))) {
                echo "<div>".select_input(" name='where[$u][col]'", $d, $X["col"],
                        ($X ? "selectFieldChange" : "selectAddRow"), "(".lang(53).")"), html_select("where[$u][op]",
                    $this->operators, $X["op"],
                    $La), "<input type='search' name='where[$u][val]' value='".h($X["val"])."'>", script("mixin(qsl('input'), {oninput: function () { $La }, onkeydown: selectSearchKeydown, onsearch: selectSearchSearch});",
                    ""), "</div>\n";
            }
        }
        echo "</div></fieldset>\n";
    ...
    selectOrderPrint(
        ...
        foreach ((array) $_GET["order"] as $_ => $X) {
            if ($X != "") {
                echo "<div>".select_input(" name='order[$u]'", $d, $X, "selectFieldChange"), checkbox("desc[$u]", 1,
                        isset($_GET["desc"][$_]), lang(55))."</div>\n";
                $u++;
            }
        }
        echo "<div>".select_input(" name='order[$u]'", $d, "", "selectAddRow"), checkbox("desc[$u]", 1, false,
                lang(55))."</div>\n", "</div></fieldset>\n";
    }
    ...
    selectLimitPrint(
        ...
        echo "<input type='number' name='limit' class='size' value='".h($A)."'>", script("qsl('input').oninput = selectFieldChange;",
            ""), "</div></fieldset>\n";
    }
        ...
        foreach ($y
            ...
            if ($x["type"] == "FULLTEXT" && $_GET["fulltext"][$u] != "") {
                $K[] = "MATCH (".implode(", ", array_map('idf_escape',
                        $x["columns"])).") AGAINST (".q($_GET["fulltext"][$u]).(isset($_GET["boolean"][$u]) ? " IN BOOLEAN MODE" : "").")";
            }
        }
                ...
                if ($X["col"] != "") {
                    ...
                    foreach ($n
                        ...
                        if ((preg_match('~^[-\d.'.(preg_match('~IN$~', $X["op"]) ? ',' : '').']+$~',
                                    ...
                                    $m["type"])) && (!preg_match("~[\x80-\xFF]~",
                                    $X["val"]) || preg_match('~char|text|enum|set~', $m["type"]))) {
                            $Za[] = $Xe.$k->convertSearch(idf_escape($F), $X, $m).$db;
                        }
                    }
        ...
        foreach ((array) $_GET["order"] as $_ => $X) {
            if ($X != "") {
                $K[] = (preg_match('~^((COUNT\(DISTINCT |[A-Z0-9_]+\()(`(?:[^`]|``)+`|"(?:[^"]|"")+")\)|COUNT\(\*\))$~',
                        $X) ? $X : idf_escape($X)).(isset($_GET["desc"][$_]) ? " DESC" : "");
            }
        }
        return $K;
    }
    ...
    messageQuery(
        ...
        $Nc[$_GET["db"]][] = array($I, time(), $Ag);
        $Zf = "sql-".count($Nc[$_GET["db"]]);
        $K = "<a href='#$Zf' class='toggle'>".lang(60)."</a>\n";
        if (!$mc && ($th = $k->warnings())) {
            $v = "warnings-".count($Nc[$_GET["db"]]);
            $K = "<a href='#$v' class='toggle'>".lang(42)."</a>, $K<div id='$v' class='hidden'>\n$th</div>\n";
        }
        return " <span class='time'>".@date("H:i:s")."</span>"." $K<div id='$Zf' class='hidden'><pre><code class='jush-$z'>".shorten_utf8($I,
                1000)."</code></pre>".($Ag ? " <span class='time'>($Ag)</span>" : '').(support("sql") ? '<p><a href="'.h(str_replace("db=".urlencode(DB),
                        "db=".urlencode($_GET["db"]),
                        ME).'sql=&history='.(count($Nc[$_GET["db"]]) - 1)).'">'.lang(10).'</a>' : '').'</div>';
    }
    ...
    editInput(
        ...
        if ($m["type"] == "enum") {
            return (isset($_GET["select"]) ? "<label><input type='radio'$xa value='-1' checked><i>".lang(8)."</i></label> " : "").($m["null"] ? "<label><input type='radio'$xa value=''".($Y !== null || isset($_GET["select"]) ? "" : " checked")."><i>NULL</i></label> " : "").enum_input("radio",
                    $xa, $m, $Y, 0);
        }
        return "";
    }
    ...
    dumpTable(
        ...
        if ($_POST["format"] != "sql") {
            ...
            set_utf8mb4($h);
            if ($hg && $h) {
                if ($hg == "DROP+CREATE" || $gd == 1) {
                    echo "DROP ".($gd == 2 ? "VIEW" : "TABLE")." IF EXISTS ".table($Q).";\n";
                }
                if ($gd == 1) {
                    $h = remove_definer($h);
                }
            ...
            if ($J) {
                ...
                while ($L = $J->$pc()) {
                    ...
                    if ($_POST["format"] != "sql") {
                        ...
                        if (!$Zc) {
                            $Zc = "INSERT INTO ".table($Q)." (".implode(", ", array_map('idf_escape', $jd)).") VALUES";
                        }
                        foreach ($L
                                 as $_ => $X) {
                            $m = $n[$_];
                            $L[$_] = ($X !== null ? unconvert_field($m,
                                preg_match(number_type(), $m["type"]) && !preg_match('~\[~',
                                    $m["full_type"]) && is_numeric($X) ? $X : q(($X === false ? 0 : $X))) : "NULL");
                        }
                        $Cf = ($Fd ? "\n" : " ")."(".implode(",\t", $L).")";
                        if (!$Ja) {
                            $Ja = $Zc.$Cf;
                        } elseif (strlen($Ja) + 4 + strlen($Cf) + strlen($jg) < $Fd) {
                            $Ja .= ",$Cf";
                        } else {
                            echo $Ja.$jg;
                            $Ja = $Zc.$Cf;
    ...
    dumpFilename(
        ...
        return
            friendly_url($Rc != "" ? $Rc : (SERVER != "" ? SERVER : "localhost"));
    }
    ...
    dumpHeaders(
        ...
        $Ce = $_POST["output"];
        $jc = (preg_match('~sql~', $_POST["format"]) ? "sql" : ($Rd ? "tar" : "csv"));
...
';
        if ($Qd == "auth") {
            $Ce = "";
            foreach ((array) $_SESSION["pwds"] as $nh => $Nf) {
                foreach ($Nf
                         as $O => $jh) {
                    foreach ($jh
                             as $V => $Ne) {
                        if ($Ne !== null) {
                            $wb = $_SESSION["db"][$nh][$O][$V];
                            foreach (($wb ? array_keys($wb) : array("")) as $j) {
                                $Ce .= "<li><a href='".h(auth_url($nh, $O, $V,
                                        $j))."'>($Hb[$nh]) ".h($V.($O != "" ? "@".$this->serverName($O) : "").($j != "" ? " - $j" : ""))."</a>\n";
                            }
                        }
                    }
                }
            ...
            echo
            script_src(preg_replace("~\\?.*~", "", ME)."?file=jush.js&version=4.7.4");
            if (support("sql")) {
                echo '<script', nonce(), '>
';
                if ($S) {
                    ...
                    echo "var jushLinks = { $z: [ '".js_escape(ME).(support("table") ? "table=" : "select=")."\$&', /\\b(".implode("|",
                            $zd).")\\b/g ] };\n";
                ...
                $Mf = $f->server_info;
                echo 'bodyLoad(\'', (is_object($f) ? preg_replace('~^(\d\.?\d).*~s', '\1',
                    $Mf) : ""), '\'', (preg_match('~MariaDB~', $Mf) ? ", true" : ""), ');
</script>
';
            }
        ...
        foreach ($S
            ...
            if ($F != "") {
                echo '<li><a href="'.h(ME).'select='.urlencode($Q).'"'.bold($_GET["select"] == $Q || $_GET["edit"] == $Q,
                        "select").">".lang(73)."</a> ", (support("table") || support("indexes") ? '<a href="'.h(ME).'table='.urlencode($Q).'"'.bold(in_array($Q,
                            array(
                                $_GET["table"], $_GET["create"], $_GET["indexes"], $_GET["foreign"], $_GET["trigger"]
                            )),
                            (is_view($cg) ? "view" : "structure"))." title='".lang(38)."'>$F</a>" : "<span>$F</span>")."\n";
            }
        }
        echo "</ul>\n";
    }
...
', script_src(preg_replace("~\\?.*~", "", ME)."?file=functions.js&version=4.7.4");
    if ($b->head()) {
        echo '<link rel="shortcut icon" type="image/x-icon" href="', h(preg_replace("~\\?.*~", "",
                ME)."?file=favicon.ico&version=4.7.4"), '">
...
';
    ...
    if (!$_COOKIE["adminer_version"] && function_exists('openssl_verify') && file_exists($rc) && filemtime($rc) + 86400 > time()) {
        $oh = unserialize(file_get_contents($rc));
        $gf = "-----BEGIN PUBLIC KEY-----
...
-----END PUBLIC KEY-----
";
        if (openssl_verify($oh["version"], base64_decode($oh["signature"]), $gf) == 1) {
            $_COOKIE["adminer_version"] = $oh["version"];
        }
    }
    echo '<script', nonce(), '>
...
page_headers()
{
    ...
    foreach ($b->csp() as $nb) {
        ...
        header("Content-Security-Policy: ".implode("; ", $Lc));
    }
    $b->headers();
}
    ...
    return
        array(
            array(
                ...
                "connect-src" => "'self'", "frame-src" => "https://www.adminer.org", "object-src" => "'none'",
...
xxtea_mx(
    ...
    return
        int32((($zh >> 5 & 0x7FFFFFF) ^ $yh << 2) + (($yh >> 3 & 0x1FFFFFFF) ^ $zh << 4)) ^ int32(($kg ^ $yh) + ($id ^ $zh));
}
...
encrypt_string(
    ...
    while ($H-- > 0) {
        ...
        for ($De = 0; $De < $Td; $De++) {
            $yh = $W[$De + 1];
            $Sd = xxtea_mx($zh, $yh, $kg, $_[$De & 3 ^ $Nb]);
            $zh = int32($W[$De] + $Sd);
            $W[$De] = $zh;
        }
        $yh = $W[0];
...
add_invalid_login()
{
    ...
    $cd = unserialize(stream_get_contents($q));
    $Ag = time();
    if ($cd) {
        foreach ($cd
                 as $dd => $X) {
    ...
    $bd =& $cd[$b->bruteForceKey()];
    if (!$bd) {
        $bd = array($Ag + 30 * 60, 0);
    }
    $bd[1]++;
    file_write_unlock($q, serialize($cd));
}
...
{
    global $b;
    $cd = unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));
    $bd = $cd[$b->bruteForceKey()];
    $Yd = ($bd[1] > 29 ? $bd[0] - time() : 0);
    if ($Yd > 0) {
        auth_error(lang(77, ceil($Yd / 60)));
    }
}
...
if ($ya) {
    ...
    $O = $ya["server"];
    $V = $ya["username"];
    $Ne = (string) $ya["password"];
    $j = $ya["db"];
    set_password($nh, $O, $V, $Ne);
    $_SESSION["db"][$nh][$O][$V][$j] = true;
    if ($ya["permanent"]) {
        $_ = base64_encode($nh)."-".base64_encode($O)."-".base64_encode($V)."-".base64_encode($j);
        $bf = $b->permanentLogin(true);
        $Qe[$_] = "$_:".base64_encode($bf ? encrypt_string($Ne, $bf) : "");
        cookie("adminer_permanent", implode(" ", $Qe));
    }
    if (count($_POST) == 1 || DRIVER != $nh || SERVER != $O || $_GET["username"] !== $V || DB != $j) {
        redirect(auth_url($nh, $O, $V, $j));
    }
} elseif ($_POST["logout"]) {
    if ($Kc && !verify_token()) {
    ...
    foreach ($Qe
             as $_ => $X) {
        list(, $Ra) = explode(":", $X);
        list($nh, $O, $V, $j) = array_map('base64_decode', explode("-", $_));
        set_password($nh, $O, $V, decrypt_string(base64_decode($Ra), $bf));
        $_SESSION["db"][$nh][$O][$V][$j] = true;
    }
}
function
unset_permanent()
{
    ...
    foreach ($Qe
             as $_ => $X) {
        list($nh, $O, $V, $j) = array_map('base64_decode', explode("-", $_));
        if ($nh == DRIVER && $O == SERVER && $V == $_GET["username"] && $j == DB) {
            unset($Qe[$_]);
        }
    }
    cookie("adminer_permanent", implode(" ", $Qe));
...
$_d = null;
if (!is_object($f) || ($_d = $b->login($_GET["username"], get_password())) !== true) {
    $l = (is_string($f) ? h($f) : (is_string($_d) ? $_d : lang(88)));
...
if ($_POST) {
    if (!verify_token()) {
        ...
        $l = (!$_POST["token"] && $Jd ? lang(90, "'$Wc'") : lang(78).' '.lang(91));
    }
    ...
    for ($u = 0; (!$A || $u < $A) && ($L = $J->fetch_row()); $u++) {
        if (!$u) {
            ...
            for ($hd = 0; $hd < count($L); $hd++) {
                ...
                $K[$m->table] = $ve;
                if ($we && $z == "sql") {
                    $zd[$hd] = ($F == "table" ? "table=" : ($F == "possible_keys" ? "indexes=" : null));
                } elseif ($ve != "") {
                    if (!isset($y[$ve])) {
                        $y[$ve] = array();
                        foreach (indexes($ve, $g) as $x) {
                            if ($x["type"] == "PRIMARY") {
                ...
                $Ug[$hd] = $m->type;
                echo "<th".($ve != "" || $m->name != $ue ? " title='".h(($ve != "" ? "$ve." : "").$ue)."'" : "").">".h($F).($we ? doc_link(array(
        ...
        foreach ($L
            ...
            if (isset($zd[$_]) && !$d[$zd[$_]]) {
                if ($we && $z == "sql") {
                    $Q = $L[array_search("table=", $zd)];
                    $B = $zd[$_].urlencode($we[$Q] != "" ? $we[$Q] : $Q);
                } else {
                    $B = "edit=".urlencode($zd[$_]);
                    foreach ($y[$zd[$_]] as $Va => $hd) {
                        $B .= "&where".urlencode("[".bracket_escape($Va)."]")."=".urlencode($L[$hd]);
                    }
                }
                $X = "<a href='".h(ME.$B)."'>$X</a>";
            }
...
textarea(
    ...
    global $z;
    echo "<textarea name='$F' rows='$M' cols='$Za' class='sqlarea jush-$z' spellcheck='false' wrap='off'>";
    if (is_array($Y)) {
        foreach ($Y
                 as $X) {
            echo
...
edit_type(
    ...
    if ($U && !isset($Ug[$U]) && !isset($p[$U]) && !in_array($U, $lc)) {
        $lc[] = $U;
    }
    if ($p) {
        $gg[lang(94)] = $p;
    ...
    echo
    optionlist(array_merge($lc, $gg), $U), '</select>
', on_help("getTarget(event).value",
        1), script("mixin(qsl('select'), {onfocus: function () { lastType = selectValue(this); }, onchange: editingTypeChange});",
        ""), '<td><input name="', h($_), '[length]" value="', h($m["length"]), '" size="3"', (!$m["length"] && preg_match('~var(char|binary)$~',
        $U) ? " class='required'" : "");
    echo ' aria-labelledby="label-length">', script("mixin(qsl('input'), {onfocus: editingLengthFocus, oninput: editingLengthChange});",
        ""), '<td class="options">', "<select name='".h($_)."[collation]'".(preg_match('~(char|text|enum|set)$~',
            $U) ? "" : " class='hidden'").'><option value="">('.lang(95).')'.optionlist($Ya,
            $m["collation"]).'</select>', ($bh ? "<select name='".h($_)."[unsigned]'".(!$U || preg_match(number_type(),
            $U) ? "" : " class='hidden'").'><option>'.optionlist($bh,
            $m["unsigned"]).'</select>' : ''), (isset($m['on_update']) ? "<select name='".h($_)."[on_update]'".(preg_match('~timestamp|datetime~',
            $U) ? "" : " class='hidden'").'>'.optionlist(array("" => "(".lang(96).")", "CURRENT_TIMESTAMP"),
            (preg_match('~^CURRENT_TIMESTAMP~i',
                $m["on_update"]) ? "CURRENT_TIMESTAMP" : $m["on_update"])).'</select>' : ''), ($p ? "<select name='".h($_)."[on_delete]'".(preg_match("~`~",
            $U) ? "" : " class='hidden'")."><option value=''>(".lang(97).")".optionlist(explode("|", $le),
            $m["on_delete"])."</select> " : " ");
}
...
process_length(
    ...
    global $Yb;
    return (preg_match("~^\\s*\\(?\\s*$Yb(?:\\s*,\\s*$Yb)*+\\s*\\)?\\s*\$~", $wd) && preg_match_all("~$Yb~", $wd,
...
process_type(
    ...
    global $bh;
    return " $m[type]".process_length($m["length"]).(preg_match(number_type(), $m["type"]) && in_array($m["unsigned"],
            $bh) ? " $m[unsigned]" : "").(preg_match('~char|text|enum|set~',
            $m["type"]) && $m["collation"] ? " $Wa ".q($m["collation"]) : "");
}
...
process_field(
    ...
    return
        array(
            idf_escape(trim($m["field"])), process_type($Sg), ($m["null"] ? " NULL" : " NOT NULL"), default_value($m),
...
default_value(
    ...
    return ($yb === null ? "" : " DEFAULT ".(preg_match('~char|binary|text|enum|set~',
            $m["type"]) || preg_match('~^(?![a-z])~i', $yb) ? q($yb) : $yb));
}
...
<td><input type="radio" name="auto_increment_col" value=""><acronym id="label-ai" title="', lang(47), '">AI</acronym>', doc_link(array(
            'sql' => "example-auto-increment.html", 'mariadb' => "auto_increment/",
        )), '<td id="label-default">', lang(48), (support("comment") ? "<td id='label-comment'>".lang(46) : "");
    }
    echo '<td>', "<input type='image' class='icon' name='add[".(support("move_col") ? 0 : count($n))."]' src='".h(preg_replace("~\\?.*~",
                "",
                ME)."?file=plus.gif&version=4.7.4")."' alt='+' title='".lang(102)."'>".script("row_count = ".count($n).";"), '</thead>
<tbody>
', script("mixin(qsl('tbody'), {onclick: editingClick, onkeydown: editingKeydown, oninput: editingInput});");
    foreach ($n
        ...
        $Eb = (isset($_POST["add"][$u - 1]) || (isset($m["field"]) && !$_POST["drop_col"][$u])) && (support("drop_col") || $xe == "");
        echo '<tr', ($Eb ? "" : " style='display: none;'"), '>
', ($U == "PROCEDURE" ? "<td>".html_select("fields[$u][inout]", explode("|", $Xc), $m["inout"]) : ""), '<th>';
        if ($Eb) {
            echo '<input name="fields[', $u, '][field]" value="', h($m["field"]), '" data-maxlength="64" autocapitalize="off" aria-labelledby="label-name">', script("qsl('input').oninput = function () { editingNameChange.call(this);".($m["field"] != "" || count($n) > 1 ? "" : " editingAddRow.call(this);")." };",
                "");
        }
        echo '<input type="hidden" name="fields[', $u, '][orig]" value="', h($xe), '">
';
        edit_type("fields[$u]", $m, $Ya, $p);
        if ($U == "TABLE") {
            echo '<td>', checkbox("fields[$u][null]", 1, $m["null"], "", "", "block",
                "label-null"), '<td><label class="block"><input type="radio" name="auto_increment_col" value="', $u, '"';
            if ($m["auto_increment"]) {
                echo ' checked';
            }
            echo ' aria-labelledby="label-ai"></label><td>', checkbox("fields[$u][has_default]", 1, $m["has_default"],
                "", "", "",
                "label-default"), '<input name="fields[', $u, '][default]" value="', h($m["default"]), '" aria-labelledby="label-default">', (support("comment") ? "<td><input name='fields[$u][comment]' value='".h($m["comment"])."' data-maxlength='".(min_version(5.5) ? 1024 : 255)."' aria-labelledby='label-comment'>" : "");
        }
        echo "<td>", (support("move_col") ? "<input type='image' class='icon' name='add[$u]' src='".h(preg_replace("~\\?.*~",
                    "",
                    ME)."?file=plus.gif&version=4.7.4")."' alt='+' title='".lang(102)."'> "."<input type='image' class='icon' name='up[$u]' src='".h(preg_replace("~\\?.*~",
                    "",
                    ME)."?file=up.gif&version=4.7.4")."' alt='↑' title='".lang(103)."'> "."<input type='image' class='icon' name='down[$u]' src='".h(preg_replace("~\\?.*~",
                    "",
                    ME)."?file=down.gif&version=4.7.4")."' alt='↓' title='".lang(104)."'> " : ""), ($xe == "" || support("drop_col") ? "<input type='image' class='icon' name='drop_col[$u]' src='".h(preg_replace("~\\?.*~",
                    "", ME)."?file=cross.gif&version=4.7.4")."' alt='x' title='".lang(105)."'>" : "");
    }
}
...
normalize_enum(
    ...
    return "'".str_replace("'", "''",
            addcslashes(stripcslashes(str_replace($D[0][0].$D[0][0], $D[0][0], substr($D[0], 1, -1))), '\\'))."'";
}
...
grant(
    ...
    if ($df == array("ALL PRIVILEGES", "GRANT OPTION")) {
        return ($s == "GRANT" ? queries("$s ALL PRIVILEGES$ke WITH GRANT OPTION") : queries("$s ALL PRIVILEGES$ke") && queries("$s GRANT OPTION$ke"));
    }
    return
        queries("$s ".preg_replace('~(GRANT OPTION)\([^)]*\)~', '\1', implode("$d, ", $df).$d).$ke);
}
...
drop_create(
    ...
    if ($_POST["drop"]) {
        query_redirect($Ib, $C, $Nd);
    } elseif ($he == "") {
        query_redirect($h, $C, $Md);
    } elseif ($he != $Wd) {
        $lb = queries($h);
        queries_redirect($C, $Ld, $lb && queries($Ib));
        if ($lb) {
            queries($Jb);
        }
    } else {
        queries_redirect($C, $Ld, queries($yg) && queries($Kb) && queries($Ib) && queries($h));
    }
}
...
create_trigger(
    ...
    return "CREATE TRIGGER ".idf_escape($L["Trigger"]).($z == "mssql" ? $ke.$Cg : $Cg.$ke).rtrim(" $L[Type]\n$L[Statement]",
            ";").";";
}
...
create_routine(
    ...
    foreach ($n
             as $m) {
        if ($m["field"] != "") {
            $P[] = (preg_match("~^($Xc)\$~", $m["inout"]) ? "$m[inout] " : "").idf_escape($m["field"]).process_type($m,
                    "CHARACTER SET");
        }
    }
    $zb = rtrim("\n$L[definition]", ";");
    return "CREATE $_f ".idf_escape(trim($L["name"]))." (".implode(", ",
            $P).")".(isset($_GET["function"]) ? " RETURNS".process_type($L["returns"],
                "CHARACTER SET") : "").($L["language"] ? " LANGUAGE $L[language]" : "").($z == "pgsql" ? " AS ".q($zb) : "$zb;");
}
...
format_foreign_key(
    ...
    return " FOREIGN KEY (".implode(", ", array_map('idf_escape',
            $o["source"])).") REFERENCES ".($j != "" && $j != $_GET["db"] ? idf_escape($j)."." : "").($ae != "" && $ae != $_GET["ns"] ? idf_escape($ae)."." : "").table($o["table"])." (".implode(", ",
            array_map('idf_escape', $o["target"])).")".(preg_match("~^($le)\$~",
            $o["on_delete"]) ? " ON DELETE $o[on_delete]" : "").(preg_match("~^($le)\$~",
            $o["on_update"]) ? " ON UPDATE $o[on_update]" : "");
}
...
tar_file(
    ...
    $K = pack("a100a8a8a8a12a12", $rc, 644, 0, 0, decoct($Hg->size), decoct(time()));
    $Qa = 8 * 32;
...
doc_link(
    ...
    global $z, $f;
    $Mf = $f->server_info;
    $oh = preg_replace('~^(\d\.?\d).*~s', '\1', $Mf);
    $fh = array(
        'sql' => "https://dev.mysql.com/doc/refman/$oh/en/", 'sqlite' => "https://www.sqlite.org/",
        'pgsql' => "https://www.postgresql.org/docs/$oh/", 'mssql' => "https://msdn.microsoft.com/library/",
    ...
    return ($Oe[$z] ? "<a href='$fh[$z]$Oe[$z]'".target_blank().">$zg</a>" : "");
}
...
connect_error()
{
    ...
    if (DB != "") {
        ...
        echo "<p class='links'>\n";
        foreach (array(
                     'database' => lang(109), 'privileges' => lang(67), 'processlist' => lang(110),
                     'variables' => lang(111), 'status' => lang(112),
                 ) as $_ => $X) {
            if (support($_)) {
        ...
        if ($i) {
            $Ff = support("scheme");
            $Ya = collations();
            echo "<form action='' method='post'>\n", "<table cellspacing='0' class='checkable'>\n", script("mixin(qsl('table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true)});"), "<thead><tr>".(support("database") ? "<td>" : "")."<th>".lang(33)." - <a href='".h(ME)."refresh=1'>".lang(115)."</a>"."<td>".lang(116)."<td>".lang(117)."<td>".lang(118)." - <a href='".h(ME)."dbsize=1'>".lang(119)."</a>".script("qsl('a').onclick = partial(ajaxSetHtml, '".js_escape(ME)."script=connect');",
                    "")."</thead>\n";
            ...
            foreach ($i
                ...
                $v = h("Db-".$j);
                echo "<tr".odd().">".(support("database") ? "<td>".checkbox("db[]", $j,
                            in_array($j, (array) $_POST["db"]), "", "", "",
                            $v) : ""), "<th><a href='$zf' id='$v'>".h($j)."</a>";
...
if (isset($_GET["download"])) {
    ...
    $F = $b->tableName($R);
    page_header(($n && is_view($R) ? $R['Engine'] == 'materialized view' ? lang(122) : lang(123) : lang(124)).": ".($F != "" ? $F : h($a)),
        $l);
    $b->selectLinks($R);
    $cb = $R["Comment"];
    if ($cb != "") {
    ...
    if (!is_view($R)) {
        ...
        if (fk_support($R)) {
            ...
            if ($p) {
                ...
                foreach ($p
                         as $F => $o) {
                    echo "<tr title='".h($F)."'>", "<th><i>".implode("</i>, <i>", array_map('h',
                            $o["source"]))."</i>", "<td><a href='".h($o["db"] != "" ? preg_replace('~db=[^&]*~',
                            "db=".urlencode($o["db"]), ME) : ($o["ns"] != "" ? preg_replace('~ns=[^&]*~',
                            "ns=".urlencode($o["ns"]),
                            ME) : ME))."table=".urlencode($o["table"])."'>".($o["db"] != "" ? "<b>".h($o["db"])."</b>." : "").($o["ns"] != "" ? "<b>".h($o["ns"])."</b>." : "").h($o["table"])."</a>", "(<i>".implode("</i>, <i>",
                            array_map('h',
                                $o["target"]))."</i>)", "<td>".h($o["on_delete"])."\n", "<td>".h($o["on_update"])."\n", '<td><a href="'.h(ME.'foreign='.urlencode($a).'&name='.urlencode($F)).'">'.lang(129).'</a>';
                }
                echo "</table>\n";
            }
    ...
    if (support(is_view($R) ? "view_trigger" : "trigger")) {
        ...
        if ($Rg) {
            ...
            foreach ($Rg
                     as $_ => $X) {
                echo "<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($_)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($_))."'>".lang(129)."</a>\n";
            }
            echo "</table>\n";
        }
    ...
    foreach (table_status('', true) as $Q => $R) {
        ...
        $Ef[$Q]["pos"] = ($pg[$Q] ? $pg[$Q] : array($Jg, 0));
        foreach ($b->foreignKeys($Q) as $X) {
            if (!$X["db"]) {
                $sd = $Da;
                if ($pg[$Q][1] || $pg[$X["table"]][1]) {
                    $sd = min(floatval($pg[$Q][1]), floatval($pg[$X["table"]][1])) - 1;
                } else {
                    $Da -= .1;
                }
                while ($ud[(string) $sd]) {
                ...
                $Ef[$Q]["references"][$X["table"]][(string) $sd] = array($X["source"], $X["target"]);
                $qf[$X["table"]][$Q][(string) $sd] = $X["target"];
                $ud[(string) $sd] = true;
            }
        }
    ...
    foreach ($Ef
        ...
        foreach ((array) $Q["references"] as $wg => $rf) {
            foreach ($rf
                ...
                $u = 0;
                foreach ($nf[0] as $Vf) {
                    echo "\n<div class='references' title='".h($wg)."' id='refs$sd-".($u++)."' style='left: $td"."em; top: ".$Q["fields"][$Vf]["pos"]."em; padding-top: .5em;'><div style='border-top: 1px solid Gray; width: ".(-$td)."em;'></div></div>";
                }
            }
        }
        foreach ((array) $qf[$F] as $wg => $rf) {
            foreach ($rf
                     as $sd => $d) {
                $td = $sd - $pg[$F][1];
                $u = 0;
                foreach ($d
                         as $vg) {
                    echo "\n<div class='references' title='".h($wg)."' id='refd$sd-".($u++)."' style='left: $td"."em; top: ".$Q["fields"][$vg]["pos"]."em; height: 1.25em; background: url(".h(preg_replace("~\\?.*~",
                                "",
                                ME)."?file=arrow.gif) no-repeat right center;&version=4.7.4")."'><div style='height: .5em; border-bottom: 1px solid Gray; width: ".(-$td)."em;'></div></div>";
                }
            }
        }
        echo "\n</div>\n";
    ...
    foreach ($Ef
        ...
        foreach ((array) $Q["references"] as $wg => $rf) {
            foreach ($rf
                ...
                $Pd = $Jg;
                $Hd = -10;
                foreach ($nf[0] as $_ => $Vf) {
                    $Ue = $Q["pos"][0] + $Q["fields"][$Vf]["pos"];
                    $Ve = $Ef[$wg]["pos"][0] + $Ef[$wg]["fields"][$nf[1][$_]]["pos"];
                    $Pd = min($Pd, $Ue, $Ve);
                    $Hd = max($Hd, $Ue, $Ve);
                }
...
';
    ...
    if ($_POST && !$l) {
        $jb = "";
        foreach (array(
                     "output", "format", "db_style", "routines", "events", "table_style", "auto_increment", "triggers",
                     "data_style"
                 ) as $_) {
        ...
        $fd = preg_match('~sql~', $_POST["format"]);
        if ($fd) {
...
";
        ...
        foreach ((array) $i
            ...
            $b->dumpDatabase($j);
            if ($f->select_db($j)) {
                if ($fd && preg_match('~CREATE~', $hg) && ($h = $f->result("SHOW CREATE DATABASE ".idf_escape($j),
                        1))) {
                    set_utf8mb4($h);
                    if ($hg == "DROP+CREATE") {
                    ...
                    if ($_POST["routines"]) {
                        foreach (array("FUNCTION", "PROCEDURE") as $_f) {
                            foreach (get_rows("SHOW $_f STATUS WHERE Db = ".q($j), null, "-- ") as $L) {
                                ...
                                set_utf8mb4($h);
                                $Be .= ($hg != 'DROP+CREATE' ? "DROP $_f IF EXISTS ".idf_escape($L["Name"]).";;\n" : "")."$h;;\n\n";
                            }
                        }
                    }
                    if ($_POST["events"]) {
                        foreach (get_rows("SHOW EVENTS", null, "-- ") as $L) {
                            ...
                            set_utf8mb4($h);
                            $Be .= ($hg != 'DROP+CREATE' ? "DROP EVENT IF EXISTS ".idf_escape($L["Name"]).";;\n" : "")."$h;;\n\n";
                        }
                    }
                    if ($Be) {
                        echo "DELIMITER ;;\n\n$Be"."DELIMITER ;\n\n";
                ...
                if ($_POST["table_style"] || $_POST["data_style"]) {
                    ...
                    foreach (table_status('', true) as $F => $R) {
                        ...
                        if ($Q || $rb) {
                            if ($jc == "tar") {
                                $Hg = new
                                TmpFile;
                                ob_start(array($Hg, 'write'), 1e5);
                            }
                            $b->dumpTable($F, ($Q ? $_POST["table_style"] : ""), (is_view($R) ? 2 : 0));
                            if (is_view($R)) {
                                $qh[] = $F;
                            } elseif ($rb) {
                                $n = fields($F);
                                $b->dumpData($F, $_POST["data_style"],
                                    "SELECT *".convert_fields($n, $n)." FROM ".table($F));
                            }
                            if ($fd && $_POST["triggers"] && $Q && ($Rg = trigger_sql($F))) {
                                echo "\nDELIMITER ;;\n$Rg\nDELIMITER ;\n";
                            }
                            if ($jc == "tar") {
                                ob_end_flush();
                                tar_file((DB != "" ? "" : "$j/")."$F.csv", $Hg);
                            } elseif ($fd) {
                                echo "\n";
                            }
                        }
                    ...
                    foreach ($qh
                             as $ph) {
                        $b->dumpTable($ph, $_POST["table_style"], 1);
                    }
                    if ($jc == "tar") {
                        echo
                        pack("x512");
...
';
    ...
    if (!$L) {
        $L = array(
            "output" => "text", "format" => "sql", "db_style" => (DB != "" ? "" : "CREATE"),
            "table_style" => "DROP+CREATE", "data_style" => "INSERT"
        );
    }
    if (!isset($L["events"])) {
    ...
    echo($z == "sqlite" ? "" : "<tr><th>".lang(33)."<td>".html_select('db_style', $vb,
            $L["db_style"]).(support("routine") ? checkbox("routines", 1, $L["routines"],
            lang(136)) : "").(support("event") ? checkbox("events", 1, $L["events"],
            lang(137)) : "")), "<tr><th>".lang(117)."<td>".html_select('table_style', $rg,
            $L["table_style"]).checkbox("auto_increment", 1, $L["auto_increment"],
            lang(47)).(support("trigger") ? checkbox("triggers", 1, $L["triggers"],
            lang(131)) : ""), "<tr><th>".lang(138)."<td>".html_select('data_style', $sb, $L["data_style"]), '</table>
...
', script("qsl('table').onclick = dumpClick;");
    ...
    if (DB != "") {
        $Oa = ($a != "" ? "" : " checked");
        echo "<thead><tr>", "<th style='text-align: left;'><label class='block'><input type='checkbox' id='check-tables'$Oa>".lang(117)."</label>".script("qs('#check-tables').onclick = partial(formCheck, /^tables\\[/);",
                ""), "<th style='text-align: right;'><label class='block'>".lang(138)."<input type='checkbox' id='check-data'$Oa></label>".script("qs('#check-data').onclick = partial(formCheck, /^data\\[/);",
                ""), "</thead>\n";
        $qh = "";
        $sg = tables_list();
        foreach ($sg
                 as $F => $U) {
            $Xe = preg_replace('~_.*~', '', $F);
            $Oa = ($a == "" || $a == (substr($a, -1) == "%" ? "$Xe%" : $F));
            $af = "<tr><td>".checkbox("tables[]", $F, $Oa, $F, "", "block");
            if ($U !== null && !preg_match('~table~i', $U)) {
                $qh .= "$af\n";
            } else {
                echo "$af<td align='right'><label class='block'><span id='Rows-".h($F)."'></span>".checkbox("data[]",
                        $F, $Oa)."</label>\n";
            }
            $Ye[$Xe]++;
        }
        ...
        echo "<thead><tr><th style='text-align: left;'>", "<label class='block'><input type='checkbox' id='check-databases'".($a == "" ? " checked" : "").">".lang(33)."</label>", script("qs('#check-databases').onclick = partial(formCheck, /^databases\\[/);",
            ""), "</thead>\n";
        $i = $b->databases();
        if ($i) {
            foreach ($i
...
';
    ...
    foreach ($Ye
             as $_ => $X) {
        if ($_ != "" && $X > 1) {
            echo ($tc ? "<p>" : " ")."<a href='".h(ME)."dump=".urlencode("$_%")."'>".h($_)."</a>";
            $tc = false;
        }
    }
    ...
    if (!$l && $_POST) {
        ...
        if (!isset($_GET["import"])) {
            ...
            $q = @fopen((file_exists($Yf) ? $Yf : "compress.zlib://$Yf.gz"), "rb");
        ...
        if (is_string($I)) {
            ...
            if ($I != "" && strlen($I) < 1e6) {
                $H = $I.(preg_match("~;[ \t\r\n]*\$~", $I) ? "" : ";");
                if (!$Nc || reset(end($Nc)) != $H) {
                    restart_session();
                    $Nc[] = array($H, time());
                    set_session("queries", $Oc);
                    stop_session();
                }
            }
            $Wf = "(?:\\s|/\\*[\s\S]*?\\*/|(?:#|-- )[^\n]*\n?|--\r?\n)";
            ...
            unset($Mb["sql"]);
            while ($I != "") {
                if (!$ee && preg_match("~^$Wf*+DELIMITER\\s+(\\S+)~i", $I, $D)) {
                    $_b = $D[1];
                    ...
                    if (!$zc && $q && !feof($q)) {
                        ...
                        $ee = $Te + strlen($zc);
                        if ($zc && rtrim($zc) != $_b) {
                            while (preg_match('('.($zc == '/*' ? '\*/' : ($zc == '[' ? ']' : (preg_match('~^-- |^#~',
                                    $zc) ? "\n" : preg_quote($zc)."|\\\\."))).'|$)s', $I, $D, PREG_OFFSET_CAPTURE,
                                $ee)) {
                                $Cf = $D[0][0];
                                if (!$Cf && $q && !feof($q)) {
                                    $I .= fread($q, 1e5);
                            ...
                            $H = substr($I, 0, $Te);
                            $bb++;
                            $af = "<pre id='sql-$bb'><code class='jush-$z'>".$b->sqlCommandQuery($H)."</code></pre>\n";
                            if ($z == "sqlite" && preg_match("~^$Wf*+ATTACH\\b~i", $H, $D)) {
                                ...
                                do {
                                    ...
                                    if ($f->error) {
                                        ...
                                        $oa = $f->affected_rows;
                                        $th = ($_POST["only_errors"] ? "" : $k->warnings());
                                        $uh = "warnings-$bb";
                                        if ($th) {
                                            $Ag .= ", <a href='#$uh'>".lang(42)."</a>".script("qsl('a').onclick = partial(toggle, '$uh');",
                                                    "");
                                        }
                                        $hc = null;
                                        $ic = "explain-$bb";
                                        if (is_object($J)) {
                                            $A = $_POST["limit"];
                                            $we = select($J, $g, array(), $A);
                                            if (!$_POST["only_errors"]) {
                                                echo "<form action='' method='post'>\n";
                                                $be = $J->num_rows;
                                                echo "<p>".($be ? ($A && $be > $A ? lang(142, $A) : "").lang(143,
                                                            $be) : ""), $Ag;
                                                if ($g && preg_match("~^($Wf|\\()*+SELECT\\b~i",
                                                        $H) && ($hc = explain($g, $H))) {
                                                    echo ", <a href='#$ic'>Explain</a>".script("qsl('a').onclick = partial(toggle, '$ic');",
                                                            "");
                                                }
                                                $v = "export-$bb";
                                                echo ", <a href='#$v'>".lang(69)."</a>".script("qsl('a').onclick = partial(toggle, '$v');",
                                                        "")."<span id='$v' class='hidden'>: ".html_select("output",
                                                        $b->dumpOutput(), $ma["output"])." ".html_select("format", $Mb,
                                                        $ma["format"])."<input type='hidden' name='query' value='".h($H)."'>"." <input type='submit' name='export' value='".lang(69)."'><input type='hidden' name='token' value='$T'></span>\n"."</form>\n";
                                            }
                                        } else {
                                            if (preg_match("~^$Wf*+(CREATE|DROP|ALTER)$Wf++(DATABASE|SCHEMA)\\b~i",
                                                $H)) {
                                                restart_session();
                                                set_session("dbs", null);
                                                stop_session();
                                            ...
                                            if (!$_POST["only_errors"]) {
                                                echo "<p class='message' title='".h($f->info)."'>".lang(144,
                                                        $oa)."$Ag\n";
                                            }
                                        }
                                        echo($th ? "<div id='$uh' class='hidden'>\n$th</div>\n" : "");
                                        if ($hc) {
            ...
            if ($Vb) {
                ...
                echo "<p class='message'>".lang(146,
                        $bb - count($ac)), " <span class='time'>(".format_time($Kg).")</span>\n";
            } elseif ($ac && $bb > 1) {
...
';
    ...
    if (!isset($_GET["import"])) {
        ...
        $Gc = (extension_loaded("zlib") ? "[.gz]" : "");
        echo(ini_bool("file_uploads") ? "SQL$Gc (&lt; ".ini_get("upload_max_filesize")."B): <input type='file' name='sql_file[]' multiple>\n$fc" : lang(150)), "</div></fieldset>\n";
        $Tc = $b->importServerPath();
        if ($Tc) {
            echo "<fieldset><legend>".lang(151)."</legend><div>", lang(152,
                "<code>".h($Tc)."$Gc</code>"), ' <input type="submit" name="webfile" value="'.lang(153).'">', "</div></fieldset>\n";
        }
        echo "<p>";
    }
    echo
        checkbox("error_stops", 1, ($_POST ? $_POST["error_stops"] : isset($_GET["import"])),
            lang(154))."\n", checkbox("only_errors", 1, ($_POST ? $_POST["only_errors"] : isset($_GET["import"])),
            lang(155))."\n", "<input type='hidden' name='token' value='$T'>\n";
    if (!isset($_GET["import"]) && $Nc) {
        print_fieldset("history", lang(156), $_GET["history"] != "");
        for ($X = end($Nc); $X; $X = prev($Nc)) {
            $_ = key($Nc);
            list($H, $Ag, $Qb) = $X;
            echo '<a href="'.h(ME."sql=&history=$_").'">'.lang(10)."</a>"." <span class='time' title='".@date('Y-m-d',
                    $Ag)."'>".@date("H:i:s",
                    $Ag)."</span>"." <code class='jush-$z'>".shorten_utf8(ltrim(str_replace("\n", " ",
                    str_replace("\r", "", preg_replace('~^(#|-- ).*~m', '', $H)))), 80,
                    "</code>").($Qb ? " <span class='time'>($Qb)</span>" : "")."<br>\n";
        }
...
';
    ...
    foreach ($n
             as $F => $m) {
        if (!isset($m["privileges"][$ch ? "update" : "insert"]) || $b->fieldName($m) == "" || $m["generated"]) {
            unset($n[$F]);
        }
    }
    if ($_POST && !$l && !isset($_GET["select"])) {
        ...
        if ($_POST["insert"]) {
            ...
            $C = ME."select=".urlencode($a);
        }
        $y = indexes($a);
        $Xg = unique_array($_GET["where"], $y);
        $jf = "\nWHERE $Z";
        if (isset($_POST["delete"])) {
            queries_redirect($C, lang(159), $k->delete($a, $jf, !$Xg));
        } else {
            $P = array();
            foreach ($n
                     as $F => $m) {
            ...
            if ($ch) {
                ...
                queries_redirect($C, lang(160), $k->update($a, $P, $jf, !$Xg));
                if (is_ajax()) {
                    page_headers();
                    page_messages($l);
                    exit;
                ...
                $J = $k->insert($a, $P);
                $rd = ($J ? last_id() : 0);
                queries_redirect($C, lang(161, ($rd ? " $rd" : "")), $J);
            }
        }
    }
    $L = null;
    ...
    if (!support("table") && !$n) {
        ...
        if ($L) {
            foreach ($L
                ...
                $n[$_] = array("field" => $_, "null" => ($_ != $k->primary), "auto_increment" => ($_ == $k->primary));
            }
        }
    }
    edit_form($a, $n, $L, $ch);
    ...
    foreach ($pf
             as $og => $m) {
        $p[str_replace("`", "``", $og)."`".str_replace("`", "``", $m["field"])] = $og;
    }
    $ze = array();
    $R = array();
    if ($a != "") {
        ...
        if ($_POST["drop"]) {
            ...
            foreach ($L["fields"] as $_ => $m) {
                ...
                if ($m["field"] != "") {
                    ...
                    $sa[] = array($m["orig"], $ff, $qa);
                    if ($ff != process_field($ye, $ye)) {
                        $n[] = array($m["orig"], $ff, $qa);
                        if ($m["orig"] != "" || $qa) {
                            $gh = true;
                        }
                    }
                    if ($o !== null) {
                        $wc[idf_escape($m["field"])] = ($a != "" && $z != "sqlite" ? "ADD" : " ").format_foreign_key(array(
                                'table' => $p[$m["type"]], 'source' => array($m["field"]),
                                'target' => array($Sg["field"]), 'on_delete' => $m["on_delete"],
                            ));
                    }
            ...
            $F = trim($L["name"]);
            queries_redirect(ME.(support("table") ? "table=" : "select=").urlencode($F), $E,
                alter_table($a, $F, ($z == "sqlite" && ($gh || $wc) ? $sa : $n), $wc,
    ...
    if (!$_POST) {
        $L = array(
            "Engine" => $_COOKIE["adminer_engine"], "fields" => array(
                array(
                    "field" => "", "type" => (isset($Ug["int"]) ? "int" : (isset($Ug["integer"]) ? "integer" : "")),
                    "on_update" => ""
                )
...
';
        ...
        echo($Xb ? "<select name='Engine'>".optionlist(array("" => "(".lang(166).")") + $Xb,
                $L["Engine"])."</select>".on_help("getTarget(event).value",
                1).script("qsl('select').onchange = helpClose;") : ""), ' ', ($Ya && !preg_match("~sqlite|mssql~",
            $z) ? html_select("Collation", array("" => "(".lang(95).")") + $Ya,
            $L["Collation"]) : ""), ' <input type="submit" value="', lang(14), '">
';
    }
    echo '
';
...
', checkbox("defaults", 1, ($_POST ? $_POST["defaults"] : adminer_setting("defaults")), lang(167),
            "columnShow(this.checked, 5)", "jsonly"), (support("comment") ? checkbox("comments", 1,
                ($_POST ? $_POST["comments"] : adminer_setting("comments")), lang(46),
...
', script("qs('#form')['defaults'].onclick();".(support("comment") ? " editingCommentsClick(qs('#form')['comments']);" : ""));
    ...
    if ($_POST && !$l && !$_POST["add"] && !$_POST["drop_col"]) {
        ...
        foreach ($L["indexes"] as $x) {
            $F = $x["name"];
            if (in_array($x["type"], $Vc)) {
                $d = array();
                $xd = array();
                $Bb = array();
                $P = array();
                ksort($x["columns"]);
                foreach ($x["columns"] as $_ => $c) {
                    if ($c != "") {
                        $wd = $x["lengths"][$_];
                        $Ab = $x["descs"][$_];
                        $P[] = idf_escape($c).($wd ? "(".(+$wd).")" : "").($Ab ? " DESC" : "");
                        $d[] = $c;
                        $xd[] = ($wd ? $wd : null);
                        $Bb[] = $Ab;
                    }
                ...
                if ($d) {
                    ...
                    if ($gc) {
                        ...
                        ksort($gc["lengths"]);
                        ksort($gc["descs"]);
                        if ($x["type"] == $gc["type"] && array_values($gc["columns"]) === $d && (!$gc["lengths"] || array_values($gc["lengths"]) === $xd) && array_values($gc["descs"]) === $Bb) {
                            unset($y[$F]);
                            continue;
                        }
                    }
...
<table cellspacing="0" class="nowrap">
<thead><tr>
<th id="label-type">', lang(174), '<th><input type="submit" class="wayoff">', lang(175), '<th id="label-name">', lang(176), '<th><noscript>', "<input type='image' class='icon' name='add[0]' src='".h(preg_replace("~\\?.*~",
                "", ME)."?file=plus.gif&version=4.7.4")."' alt='+' title='".lang(102)."'>", '</noscript>
</thead>
';
    if ($Ze) {
        echo "<tr><td>PRIMARY<td>";
        foreach ($Ze["columns"] as $_ => $c) {
            echo
    ...
    foreach ($L["indexes"] as $x) {
        if (!$_POST["drop_col"] || $hd != key($_POST["drop_col"])) {
            echo "<tr><td>".html_select("indexes[$hd][type]", array(-1 => "") + $Vc, $x["type"],
                    ($hd == count($L["indexes"]) ? "indexesAddRow.call(this);" : 1), "label-type"), "<td>";
            ksort($x["columns"]);
            $u = 1;
            foreach ($x["columns"] as $_ => $c) {
                echo "<span>".select_input(" name='indexes[$hd][columns][$u]' title='".lang(44)."'",
                        ($n ? array_combine($n, $n) : $n), $c,
                        "partial(".($u == count($x["columns"]) ? "indexesAddColumn" : "indexesChangeColumn").", '".js_escape($z == "sql" ? "" : $_GET["indexes"]."_")."')"), ($z == "sql" || $z == "mssql" ? "<input type='number' name='indexes[$hd][lengths][$u]' class='size' value='".h($x["lengths"][$_])."' title='".lang(100)."'>" : ""), (support("descidx") ? checkbox("indexes[$hd][descs][$u]",
                    1, $x["descs"][$_], lang(55)) : ""), " </span>";
                $u++;
            }
            echo "<td><input name='indexes[$hd][name]' value='".h($x["name"])."' autocapitalize='off' aria-labelledby='label-name'>\n", "<td><input type='image' class='icon' name='drop_col[$hd]' src='".h(preg_replace("~\\?.*~",
                        "",
                        ME)."?file=cross.gif&version=4.7.4")."' alt='x' title='".lang(105)."'>".script("qsl('input').onclick = partial(editingRemoveRow, 'indexes\$1[type]');");
        }
        $hd++;
    }
    echo '</table>
...
';
    ...
    if ($_POST && !$l && !isset($_POST["add_x"])) {
        ...
        if ($_POST["drop"]) {
            ...
            if (DB != "") {
                $_GET["db"] = $F;
                queries_redirect(preg_replace('~\bdb=[^&]*&~', '', ME)."db=".urlencode($F), lang(178),
                    rename_database($F, $L["collation"]));
            } else {
                $i = explode("\n", str_replace("\r", "", $F));
                $ig = true;
                $qd = "";
                ...
                restart_session();
                set_session("dbs", null);
                queries_redirect(ME."db=".urlencode($qd), lang(179), $ig);
            }
        } else {
            if (!$L["collation"]) {
                redirect(substr(ME, 0, -1));
            }
            query_redirect("ALTER DATABASE ".idf_escape($F).(preg_match('~^[a-z0-9_]+$~i',
                    $L["collation"]) ? " COLLATE $L[collation]" : ""), substr(ME, 0, -1), lang(180));
        }
    }
...
', ($_POST["add_x"] || strpos($F,
            "\n") ? '<textarea id="name" name="name" rows="10" cols="40">'.h($F).'</textarea><br>' : '<input name="name" id="name" value="'.h($F).'" data-maxlength="64" autocapitalize="off">')."\n".($Ya ? html_select("collation",
                array("" => "(".lang(95).")") + $Ya, $L["collation"]).doc_link(array(
                'sql' => "charset-charsets.html", 'mariadb' => "supported-character-sets-and-collations/",
...
';
    if (DB != "") {
        ...
        echo "<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~", "",
                    ME)."?file=plus.gif&version=4.7.4")."' alt='+' title='".lang(102)."'>\n";
    }
    echo '<input type="hidden" name="token" value="', $T, '">
</form>
';
    ...
    $Uc = array();
    $Be = array();
    foreach ($_f["fields"] as $u => $m) {
        if (substr($m["inout"], -3) == "OUT") {
            $Be[$u] = "@".idf_escape($m["field"])." AS ".idf_escape($m["field"]);
        }
    ...
    if (!$l && $_POST) {
        ...
        foreach ($_f["fields"] as $_ => $m) {
            ...
            $Ka[] = (isset($Be[$_]) ? "@".idf_escape($m["field"]) : $X);
        }
...
';
    ...
    if ($_POST && !$l && !$_POST["add"] && !$_POST["change"] && !$_POST["change-js"]) {
        ...
        if ($z == "sqlite") {
            queries_redirect($C, $E, recreate_table($a, $a, array(), array(),
                array(" $F" => ($_POST["drop"] ? "" : " ".format_foreign_key($L)))));
        } else {
            $ta = "ALTER TABLE ".table($a);
            $Ib = "\nDROP ".($z == "sql" ? "FOREIGN KEY " : "CONSTRAINT ").idf_escape($F);
            if ($_POST["drop"]) {
                query_redirect($ta.$Ib, $C, $E);
            } else {
                query_redirect($ta.($F != "" ? "$Ib," : "")."\nADD".format_foreign_key($L), $C, $E);
                $l = lang(186)."<br>$l";
            }
        }
    }
...
';
    ...
    $vg = ($a === $L["table"] ? $Vf : array_keys(fields(in_array($L["table"], $of) ? $L["table"] : reset($of))));
    $me = "this.form['change-js'].value = '1'; this.form.submit();";
    echo "<p>".lang(188).": ".html_select("table", $of, $L["table"], $me)."\n";
    if ($z == "pgsql") {
        echo
            lang(189).": ".html_select("ns", $b->schemas(), $L["ns"] != "" ? $L["ns"] : $_GET["ns"], $me);
    } elseif ($z != "sqlite") {
        $wb = array();
        ...
        echo
            lang(72).": ".html_select("db", $wb, $L["db"] != "" ? $L["db"] : $_GET["db"], $me);
    }
...
';
    $hd = 0;
    foreach ($L["source"] as $_ => $X) {
        echo "<tr>", "<td>".html_select("source[".(+$_)."]", array(-1 => "") + $Vf, $X,
                ($hd == count($L["source"]) - 1 ? "foreignAddRow.call(this);" : 1),
                "label-source"), "<td>".html_select("target[".(+$_)."]", $vg, $L["target"][$_], 1, "label-target");
        $hd++;
    }
    echo '</table>
<p>
', lang(97), ': ', html_select("on_delete", array(-1 => "") + explode("|", $le),
        $L["on_delete"]), ' ', lang(96), ': ', html_select("on_update", array(-1 => "") + explode("|", $le),
        $L["on_update"]), doc_link(array(
        'sql' => "innodb-foreign-key-constraints.html", 'mariadb' => "foreign-keys/",
    )), '<p>
<input type="submit" value="', lang(14), '">
...
';
    ...
    if ($_POST && !$l) {
        ...
        if (!$_POST["drop"] && $a == $F && $z != "sqlite" && $U == "VIEW" && $_e == "VIEW") {
            query_redirect(($z == "mssql" ? "ALTER" : "CREATE OR REPLACE")." VIEW ".table($F).$va, $C, $E);
        } else {
            $xg = $F."_adminer_".uniqid();
            drop_create("DROP $_e ".table($a), "CREATE $U ".table($F).$va, "DROP $U ".table($F),
                "CREATE $U ".table($xg).$va, "DROP $U ".table($xg), ($_POST["drop"] ? substr(ME, 0, -1) : $C),
                lang(193), $E, lang(194), $a, $F);
        }
    }
    if (!$_POST && $a != "") {
        $L = view($a);
...
';
    ...
    $dg = array("ENABLED" => "ENABLE", "DISABLED" => "DISABLE", "SLAVESIDE_DISABLED" => "DISABLE ON SLAVE");
    $L = $_POST;
    if ($_POST && !$l) {
        if ($_POST["drop"]) {
            query_redirect("DROP EVENT ".idf_escape($aa), substr(ME, 0, -1), lang(196));
        } elseif (in_array($L["INTERVAL_FIELD"], $ad) && isset($dg[$L["STATUS"]])) {
...
<tr><th>', lang(203), '<td><input type="number" name="INTERVAL_VALUE" value="', h($L["INTERVAL_VALUE"]), '" class="size"> ', html_select("INTERVAL_FIELD",
        $ad, $L["INTERVAL_FIELD"]), '<tr><th>', lang(112), '<td>', html_select("STATUS", $dg,
        $L["STATUS"]), '<tr><th>', lang(46), '<td><input name="EVENT_COMMENT" value="', h($L["EVENT_COMMENT"]), '" data-maxlength="64">
...
';
    ...
    if ($_POST && !process_fields($L["fields"]) && !$l) {
        ...
        drop_create("DROP $_f ".routine_id($da, $xe), create_routine($_f, $L), "DROP $_f ".routine_id($L["name"], $L),
            create_routine($_f, array("name" => $xg) + $L), "DROP $_f ".routine_id($xg, $L), substr(ME, 0, -1),
            lang(205), lang(206), lang(207), $da, $L["name"]);
    }
    page_header(($da != "" ? (isset($_GET["function"]) ? lang(208) : lang(209)).": ".h($da) : (isset($_GET["function"]) ? lang(210) : lang(211))),
        $l);
    if (!$_POST && $da != "") {
...
<form action="" method="post" id="form">
<p>', lang(176), ': <input name="name" value="', h($L["name"]), '" data-maxlength="64" autocapitalize="off">
', ($Af ? lang(19).": ".html_select("language", $Af,
            $L["language"])."\n" : ""), '<input type="submit" value="', lang(14), '">
<div class="scrollable">
<table cellspacing="0" class="nowrap">
';
    ...
    if (isset($_GET["function"])) {
        echo "<tr><td>".lang(212);
        edit_type("returns", $L["returns"], $Ya, array(), ($z == "pgsql" ? array("void", "trigger") : array()));
    }
    echo '</table>
</div>
<p>';
...
';
    ...
    $F = $_GET["name"];
    $Qg = trigger_options();
    $L = (array) trigger($F) + array("Trigger" => $a."_bi");
    if ($_POST) {
        if (!$l && in_array($_POST["Timing"], $Qg["Timing"]) && in_array($_POST["Event"],
                $Qg["Event"]) && in_array($_POST["Type"], $Qg["Type"])) {
            $ke = " ON ".table($a);
            $Ib = "DROP TRIGGER ".idf_escape($F).($z == "pgsql" ? $ke : "");
            $C = ME."table=".urlencode($a);
            if ($_POST["drop"]) {
                query_redirect($Ib, $C, lang(213));
            } else {
                if ($F != "") {
                    queries($Ib);
                }
                queries_redirect($C, ($F != "" ? lang(214) : lang(215)), queries(create_trigger($ke, $_POST)));
                if ($F != "") {
                    queries(create_trigger($ke, $L + array("Type" => reset($Qg["Type"]))));
                }
            }
        }
        $L = $_POST;
    }
    page_header(($F != "" ? lang(216).": ".h($F) : lang(217)), $l, array("table" => $a));
    echo '
<form action="" method="post" id="form">
<table cellspacing="0" class="layout">
<tr><th>', lang(218), '<td>', html_select("Timing", $Qg["Timing"], $L["Timing"], "triggerChange(/^".preg_quote($a,
            "/")."_[ba][iud]$/, '".js_escape($a)."', this.form);"), '<tr><th>', lang(219), '<td>', html_select("Event",
        $Qg["Event"], $L["Event"], "this.form['Timing'].onchange();"), (in_array("UPDATE OF",
        $Qg["Event"]) ? " <input name='Of' value='".h($L["Of"])."' class='hidden'>" : ""), '<tr><th>', lang(45), '<td>', html_select("Type",
        $Qg["Type"], $L["Type"]), '</table>
...
';
    ...
    if ($_POST) {
        foreach ($_POST["objects"] as $_ => $X) {
            $Vd[$X] = (array) $Vd[$X] + (array) $_POST["grants"][$_];
        }
    }
    $Cc = array();
    $ie = "";
    if (isset($_GET["host"]) && ($J = $f->query("SHOW GRANTS FOR ".q($fa)."@".q($_GET["host"])))) {
        while ($L = $J->fetch_row()) {
            if (preg_match('~GRANT (.*) ON (.*) TO ~', $L[0], $D) && preg_match_all('~ *([^(,]*[^ ,(])( *\([^)]+\))?~',
                    $D[1], $Dd, PREG_SET_ORDER)) {
                foreach ($Dd
                         as $X) {
                    if ($X[1] != "USAGE") {
                        $Cc["$D[2]$X[2]"][$X[1]] = true;
    ...
    if ($_POST && !$l) {
        ...
        if ($_POST["drop"]) {
            query_redirect("DROP USER $je", ME."privileges=", lang(220));
        } else {
            ...
            if (!$l) {
                ...
                foreach ($Vd
                    ...
                    $s = array_keys($s);
                    if (isset($_GET["grant"])) {
                        $xf = array_diff(array_keys(array_filter($Vd[$de], 'strlen')), $s);
                    } elseif ($je == $Xd) {
                        $ge = array_keys((array) $Cc[$de]);
                        $xf = array_diff($ge, $s);
                        $s = array_diff($s, $ge);
                        unset($Cc[$de]);
                    }
                    if (preg_match('~^(.+)\s*(\(.*\))?$~U', $de, $D) && (!grant("REVOKE", $xf, $D[2],
                                " ON $D[1] FROM $Xd") || !grant("GRANT", $s, $D[2], " ON $D[1] TO $Xd"))) {
                        $l = true;
                        break;
                    }
                }
                ...
                if ($je != $Xd) {
                    ...
                    foreach ($Cc
                        ...
                        if (preg_match('~^(.+)(\(.*\))?$~U', $de, $D)) {
                            grant("REVOKE", array_keys($xf), $D[2], " ON $D[1] FROM $Xd");
                        }
                    }
                }
            }
...
';
    ...
    echo(min_version(8) ? "" : checkbox("hashed", 1, $L["hashed"], lang(223),
...
';
    ...
    echo "</thead>\n";
    foreach (array(
                 "" => "", "Server Admin" => lang(30), "Databases" => lang(33), "Tables" => lang(124),
                 "Columns" => lang(44), "Procedures" => lang(224),
             ) as $hb => $Ab) {
        foreach ((array) $df[$hb] as $cf => $cb) {
            ...
            foreach ($Cc
                ...
                $Y = $s[strtoupper($cf)];
                if ($hb == "Server Admin" && $de != (isset($Cc["*.*"]) ? "*.*" : ".*")) {
                    echo "<td>";
                } elseif (isset($_GET["grant"])) {
                    echo "<td><select name=$F><option><option value='1'".($Y ? " selected" : "").">".lang(225)."<option value='0'".($Y == "0" ? " selected" : "").">".lang(226)."</select>";
                } else {
...
';
    ...
    if (support("kill") && $_POST && !$l) {
        ...
        queries_redirect(ME."processlist=", lang(227, $ld), $ld || !$_POST["kill"]);
    }
...
', script("mixin(qsl('table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true)});");
    ...
    foreach (process_list() as $u => $L) {
        if (!$u) {
            ...
            foreach ($L
                     as $_ => $X) {
                echo "<th>$_".doc_link(array('sql' => "show-processlist.html#processlist_".strtolower($_),));
            }
            echo "</thead>\n";
        }
        ...
        foreach ($L
                 as $_ => $X) {
            echo "<td>".(($z == "sql" && $_ == "Info" && preg_match("~Query|Killed~",
                        $L["Command"]) && $X != "") || ($z == "pgsql" && $_ == "current_query" && $X != "<IDLE>") || ($z == "oracle" && $_ == "sql_text" && $X != "") ? "<code class='jush-$z'>".shorten_utf8($X,
                        100,
                        "</code>").' <a href="'.h(ME.($L["db"] != "" ? "db=".urlencode($L["db"])."&" : "")."sql=".urlencode($X)).'">'.lang(228).'</a>' : h($X));
        }
        echo "\n";
    }
    echo '</table>
...
';
    if (support("kill")) {
        echo ($u + 1)."/".lang(229, max_connections()), "<p><input type='submit' value='".lang(230)."'>\n";
    }
    echo '<input type="hidden" name="token" value="', $T, '">
</form>
', script("tableCheck();");
    ...
    foreach ($n
        ...
        if (isset($m["privileges"]["select"]) && $F != "") {
            $d[$_] = html_entity_decode(strip_tags($F), ENT_QUOTES);
            if (is_shortable($m)) {
    ...
    list($N, $t) = $b->selectColumnsProcess($d, $y);
    $ed = count($t) < count($N);
    $Z = $b->selectSearchProcess($n, $y);
    $te = $b->selectOrderProcess($n, $y);
    $A = $b->selectLimitProcess();
    if ($_GET["val"] && is_ajax()) {
        ...
        if ($_POST["export"]) {
            ...
            if (!is_array($_POST["check"]) || $Ze) {
                ...
                foreach ($_POST["check"] as $X) {
                    $Wg[] = "(SELECT".limit($Ac,
                            "\nWHERE ".($Z ? implode(" AND ", $Z)." AND " : "").where_check($X, $n).$Ec, 1).")";
                }
        ...
        if (!$b->selectEmailProcess($Z, $p)) {
            if ($_POST["save"] || $_POST["delete"]) {
                ...
                if ($_POST["delete"] || $P) {
                    ...
                    if ($_POST["all"] || ($Ze && is_array($_POST["check"])) || $ed) {
                        $J = ($_POST["delete"] ? $k->delete($a,
                            $wh) : ($_POST["clone"] ? queries("INSERT $I$wh") : $k->update($a, $P, $wh)));
                        $oa = $f->affected_rows;
                    } else {
                        foreach ((array) $_POST["check"] as $X) {
                            $vh = "\nWHERE ".($Z ? implode(" AND ", $Z)." AND " : "").where_check($X, $n);
                            $J = ($_POST["delete"] ? $k->delete($a, $vh,
                                1) : ($_POST["clone"] ? queries("INSERT".limit1($a, $I, $vh)) : $k->update($a, $P, $vh,
                                1)));
                            if (!$J) {
                                break;
                            }
                ...
                if (!$_POST["val"]) {
                    ...
                    foreach ($_POST["val"] as $Yg => $L) {
                        ...
                        foreach ($L
                                 as $_ => $X) {
                            $_ = bracket_escape($_, 1);
                            $P[idf_escape($_)] = (preg_match('~char|text~',
                                $n[$_]["type"]) || $X != "" ? $b->processInput($n[$_], $X) : "NULL");
                        }
                        $J = $k->update($a, $P, " WHERE ".($Z ? implode(" AND ", $Z)." AND " : "").where_check($Yg, $n),
                            !$ed && !$Ze, " ");
                        if (!$J) {
                            break;
                        }
                ...
                $M = array();
                foreach ($Dd[0] as $_ => $X) {
                    preg_match_all("~((?>\"[^\"]*\")+|[^$Lf]*)$Lf~", $X.$Lf, $Ed);
                    if (!$_ && !array_diff($Ed[1], $Za)) {
                        $Za = $Ed[1];
                        $oa--;
                    } else {
                        $P = array();
                        foreach ($Ed[1] as $u => $Va) {
                            $P[idf_escape($Za[$u])] = ($Va == "" && $n[$Za[$u]]["null"] ? "NULL" : q(str_replace('""',
                                '"', preg_replace('~^"|"$~', '', $Va))));
                        }
                        $M[] = $P;
                    }
    ...
    if (isset($yf["insert"]) || !support("table")) {
        ...
        foreach ((array) $_GET["where"] as $X) {
            if ($p[$X["col"]] && count($p[$X["col"]]) == 1 && ($X["op"] == "=" || (!$X["op"] && !preg_match('~[_%]~',
                            $X["val"])))) {
    ...
    if (!$d && support("table")) {
        ...
        $b->selectSearchPrint($Z, $d, $y);
        $b->selectOrderPrint($te, $d, $y);
        $b->selectLimitPrint($A);
        $b->selectLengthPrint($_g);
        $b->selectActionPrint($y);
        echo "</form>\n";
        $G = $_GET["page"];
        if ($G == "last") {
            $_c = $f->result(count_rows($a, $Z, $ed, $t));
            $G = floor(max(0, $_c - 1) / $A);
        }
        $Gf = $N;
        $Dc = $t;
        if (!$Gf) {
        ...
        $J = $k->select($a, $Gf, $Z, $Dc, $te, $A, $G, true);
        if (!$J) {
            echo "<p class='error'>".error()."\n";
        } else {
            if ($z == "mssql" && $G) {
            ...
            if ($_GET["page"] != "last" && $A != "" && $t && $ed && $z == "sql") {
            ...
            if (!$M) {
                ...
                echo "<div class='scrollable'>", "<table id='table' cellspacing='0' class='nowrap checkable'>", script("mixin(qs('#table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true), onkeydown: editingKeydown});"), "<thead><tr>".(!$t && $N ? "" : "<td><input type='checkbox' id='all-page' class='jsonly'>".script("qs('#all-page').onclick = partial(formCheck, /check/);",
                ...
                foreach ($M[0] as $_ => $X) {
                    if (!isset($ah[$_])) {
                        ...
                        $F = ($m ? $b->fieldName($m, $lf) : ($X["fun"] ? "*" : $_));
                        if ($F != "") {
                            $lf++;
                            $Ud[$_] = $F;
                            $c = idf_escape($_);
                            $Qc = remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($_);
                            $Ab = "&desc%5B0%5D=1";
                            echo "<th>".script("mixin(qsl('th'), {onmouseover: partial(columnMouse), onmouseout: partial(columnMouse, ' hidden')});",
                                    ""), '<a href="'.h($Qc.($te[0] == $c || $te[0] == $_ || (!$te && $ed && $t[0] == $c) ? $Ab : '')).'">';
                            echo
                                apply_sql_function($X["fun"], $F)."</a>";
                            echo "<span class='column hidden'>", "<a href='".h($Qc.$Ab)."' title='".lang(55)."' class='text'> ↓</a>";
                            if (!$X["fun"]) {
                ...
                if ($_GET["modify"]) {
                    foreach ($M
                        ...
                        foreach ($L
                                 as $_ => $X) {
                            $xd[$_] = max($xd[$_], min(40, strlen(utf8_decode($X))));
                        }
                    }
                }
                ...
                foreach ($b->rowDescriptions($M, $p) as $Td => $L) {
                    $Xg = unique_array($M[$Td], $y);
                    if (!$Xg) {
                        $Xg = array();
                        foreach ($M[$Td] as $_ => $X) {
                    ...
                    foreach ($Xg
                             as $_ => $X) {
                        if (($z == "sql" || $z == "pgsql") && preg_match('~char|text|enum|set~',
                                $n[$_]["type"]) && strlen($X) > 64) {
                            $_ = (strpos($_, '(') ? $_ : idf_escape($_));
                            $_ = "MD5(".($z != 'sql' || preg_match("~^utf8~",
                                    $n[$_]["collation"]) ? $_ : "CONVERT($_ USING ".charset($f).")").")";
                            $X = md5($X);
                        }
                        $Yg .= "&".($X !== null ? urlencode("where[".bracket_escape($_)."]")."=".urlencode($X) : "null%5B%5D=".urlencode($_));
                    }
                    echo "<tr".odd().">".(!$t && $N ? "" : "<td>".checkbox("check[]", substr($Yg, 1),
                                in_array(substr($Yg, 1),
                                    (array) $_POST["check"])).($ed || information_schema(DB) ? "" : " <a href='".h(ME."edit=".urlencode($a).$Yg)."' class='edit'>".lang(238)."</a>"));
                    foreach ($L
                             as $_ => $X) {
                        if (isset($Ud[$_])) {
                            $m = $n[$_];
                            ...
                            if (preg_match('~blob|bytea|raw|file~', $m["type"]) && $X != "") {
                                $B = ME.'download='.urlencode($a).'&field='.urlencode($_).$Yg;
                            }
                            if (!$B && $X !== null) {
                                foreach ((array) $p[$_] as $o) {
                                    if (count($p[$_]) == 1 || end($o["source"]) == $_) {
                                        $B = "";
                                        foreach ($o["source"] as $u => $Vf) {
                                            $B .= where_link($u, $o["target"][$u], $M[$Td][$Vf]);
                                        }
                                        $B = ($o["db"] != "" ? preg_replace('~([?&]db=)[^&]+~',
                                                '\1'.urlencode($o["db"]), ME) : ME).'select='.urlencode($o["table"]).$B;
                                        if ($o["ns"]) {
                            ...
                            $X = select_value($X, $B, $m, $_g);
                            $v = h("val[$Yg][".bracket_escape($_)."]");
                            $Y = $_POST["val"][$Yg][bracket_escape($_)];
                            $Pb = !is_array($L[$_]) && is_utf8($X) && $M[$Td][$_] == $L[$_] && !$Bc[$_];
                            $zg = preg_match('~text|lob~', $m["type"]);
                            if (($_GET["modify"] && $Pb) || $Y !== null) {
                                $Hc = h($Y !== null ? $Y : $L[$_]);
                                echo "<td>".($zg ? "<textarea name='$v' cols='30' rows='".(substr_count($L[$_],
                                                "\n") + 1)."'>$Hc</textarea>" : "<input name='$v' value='$Hc' size='$xd[$_]'>");
                            } else {
                                $Ad = strpos($X, "<i>…</i>");
                                echo "<td id='$v' data-text='".($Ad ? 2 : ($zg ? 1 : 0))."'".($Pb ? "" : " data-warning='".h(lang(239))."'").">$X</td>";
                            }
                        }
                    }
                    if ($Ca) {
            ...
            if (!is_ajax()) {
                if ($M || $G) {
                    ...
                    if ($_GET["page"] != "last") {
                        if ($A == "" || (count($M) < $A && ($M || !$G))) {
                            ...
                            $_c = ($ed ? false : found_rows($R, $Z));
                            if ($_c < max(1e4, 2 * ($G + 1) * $A)) {
                                $_c = reset(slow_query(count_rows($a, $Z, $ed, $t)));
                            } else {
                                $ec = false;
                            }
                        }
                    }
                    $Ee = ($A != "" && ($_c === false || $_c > $A || $G));
                    if ($Ee) {
                        echo(($_c === false ? count($M) + 1 : $_c - $G * $A) > $A ? '<p><a href="'.h(remove_from_uri("page")."&page=".($G + 1)).'" class="loadmore">'.lang(240).'</a>'.script("qsl('a').onclick = partial(selectLoadMore, ".(+$A).", '".lang(241)."…');",
                                "") : ''), "\n";
                    }
                }
                ...
                if ($M || $G) {
                    if ($Ee) {
                        $Gd = ($_c === false ? $G + (count($M) >= $A ? 2 : 1) : floor(($_c - 1) / $A));
                        echo "<fieldset>";
                        if ($z != "simpledb") {
                            echo "<legend><a href='".h(remove_from_uri("page"))."'>".lang(242)."</a></legend>", script("qsl('a').onclick = function () { pageClick(this.href, +prompt('".lang(242)."', '".($G + 1)."')); return false; };"), pagination(0,
                                    $G).($G > 5 ? " …" : "");
                            for ($u = max(1, $G - 4); $u < min($Gd, $G + 5); $u++) {
                                echo
                                pagination($u, $G);
                            }
                            if ($Gd > 0) {
                                echo($G + 5 < $Gd ? " …" : ""), ($ec && $_c !== false ? pagination($Gd,
                                    $G) : " <a href='".h(remove_from_uri("page")."&page=last")."' title='~$Gd'>".lang(243)."</a>");
                            }
                        } else {
                            echo "<legend>".lang(242)."</legend>", pagination(0,
                                    $G).($G > 1 ? " …" : ""), ($G ? pagination($G,
                                $G) : ""), ($Gd > $G ? pagination($G + 1, $G).($Gd > $G + 1 ? " …" : "") : "");
                        }
                        echo "</fieldset>\n";
                    }
                    ...
                    $Fb = ($ec ? "" : "~ ").$_c;
                    echo
                        checkbox("all", 1, 0, ($_c !== false ? ($ec ? "" : "~ ").lang(143, $_c) : ""),
                            "var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$Fb' : checked); selectCount('selected2', this.checked || !checked ? '$Fb' : checked);")."\n", "</fieldset>\n";
...
';
                    ...
                    if ($yc) {
                        ...
                        $Ce = $b->dumpOutput();
                        echo($Ce ? html_select("output", $Ce, $na["output"])." " : ""), html_select("format", $yc,
                            $na["format"]), " <input type='submit' name='export' value='".lang(69)."'>\n", "</div></fieldset>\n";
                    }
                    $b->selectEmailPrint(array_filter($Ub, 'strlen'), $d);
                }
                echo "</div></div>\n";
                if ($b->selectImportPrint()) {
                    echo "<div>", "<a href='#import'>".lang(68)."</a>", script("qsl('a').onclick = partial(toggle, 'import');",
                        ""), "<span id='import' class='hidden'>: ", "<input type='file' name='csv_file'> ", html_select("separator",
                        array("csv" => "CSV,", "csv;" => "CSV;", "tsv" => "TSV"), $na["format"], 1);
    ...
    $mh = ($cg ? show_status() : show_variables());
    if (!$mh) {
        echo "<p class='message'>".lang(12)."\n";
    } else {
        echo "<table cellspacing='0'>\n";
    ...
    header("Content-Type: text/javascript; charset=utf-8");
    if ($_GET["script"] == "db") {
        $lg = array("Data_length" => 0, "Index_length" => 0, "Data_free" => 0);
        foreach (table_status() as $F => $R) {
            ...
            if (!is_view($R)) {
                ...
                foreach ($lg + array("Auto_increment" => 0, "Rows" => 0) as $_ => $X) {
                    if ($R[$_] != "") {
                        $X = format_number($R[$_]);
                        json_row("$_-$F",
                            ($_ == "Rows" && $X && $R["Engine"] == ($Xf == "pgsql" ? "table" : "InnoDB") ? "~ $X" : $X));
                        if (isset($lg[$_])) {
    ...
    if ($tg && !$l && !$_POST["search"]) {
        ...
        if ($_POST["truncate"]) {
            ...
            $J = ($z == "sqlite" ? queries("VACUUM") : apply_queries("VACUUM".($_POST["optimize"] ? "" : " ANALYZE"),
                $_POST["tables"]));
            $E = lang(249);
        } elseif (!$_POST["tables"]) {
            $E = lang(9);
    ...
    if ($b->homepage()) {
        if ($_GET["ns"] !== "") {
            ...
            if (!$sg) {
                ...
                echo "<form action='' method='post'>\n";
                if (support("table")) {
                    echo "<fieldset><legend>".lang(251)." <span id='selected2'></span></legend><div>", "<input type='search' name='query' value='".h($_POST["query"])."'>", script("qsl('input').onkeydown = partialArg(bodyKeydown, 'search');",
                        ""), " <input type='submit' name='search' value='".lang(52)."'>\n", "</div></fieldset>\n";
                ...
                echo "<div class='scrollable'>\n", "<table cellspacing='0' class='nowrap checkable'>\n", script("mixin(qsl('table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true)});"), '<thead><tr class="wrap">', '<td><input id="check-all" type="checkbox" class="jsonly">'.script("qs('#check-all').onclick = partial(formCheck, /^(tables|views)\[/);",
                        ""), '<th>'.lang(124), '<td>'.lang(252).doc_link(array('sql' => 'storage-engines.html')), '<td>'.lang(116).doc_link(array(
                        'sql' => 'charset-charsets.html', 'mariadb' => 'supported-character-sets-and-collations/'
                    )), '<td>'.lang(253).doc_link(array('sql' => 'show-table-status.html',)), '<td>'.lang(254).doc_link(array('sql' => 'show-table-status.html',)), '<td>'.lang(255).doc_link(array('sql' => 'show-table-status.html')), '<td>'.lang(47).doc_link(array(
                        'sql' => 'example-auto-increment.html', 'mariadb' => 'auto_increment/'
                    )), '<td>'.lang(256).doc_link(array('sql' => 'show-table-status.html',)), (support("comment") ? '<td>'.lang(46).doc_link(array('sql' => 'show-table-status.html',)) : ''), "</thead>\n";
                $S = 0;
                foreach ($sg
                         as $F => $U) {
                    $ph = ($U !== null && !preg_match('~table~i', $U));
                    $v = h("Table-".$F);
                    echo '<tr'.odd().'><td>'.checkbox(($ph ? "views[]" : "tables[]"), $F, in_array($F, $tg, true), "",
                            "", "",
                            $v), '<th>'.(support("table") || support("indexes") ? "<a href='".h(ME)."table=".urlencode($F)."' title='".lang(38)."' id='$v'>".h($F).'</a>' : h($F));
                    if ($ph) {
                        echo '<td colspan="6"><a href="'.h(ME)."view=".urlencode($F).'" title="'.lang(39).'">'.(preg_match('~materialized~i',
                                $U) ? lang(122) : lang(123)).'</a>', '<td align="right"><a href="'.h(ME)."select=".urlencode($F).'" title="'.lang(37).'">?</a>';
                    } else {
                        foreach (array(
                                     "Engine" => array(), "Collation" => array(),
                                     "Data_length" => array("create", lang(40)),
                                     "Index_length" => array("indexes", lang(126)),
                                     "Data_free" => array("edit", lang(41)),
                                     "Auto_increment" => array("auto_increment=1&create", lang(40)),
                                     "Rows" => array("select", lang(37)),
                                 ) as $_ => $B) {
                            $v = " id='$_-".h($F)."'";
                ...
                echo "<tr><td><th>".lang(229,
                        count($sg)), "<td>".h($z == "sql" ? $f->result("SELECT @@storage_engine") : ""), "<td>".h(db_collation(DB,
                        collations()));
                ...
                if (!information_schema(DB)) {
                    echo "<div class='footer'><div>\n";
                    $kh = "<input type='submit' value='".lang(257)."'> ".on_help("'VACUUM'");
                    $qe = "<input type='submit' name='optimize' value='".lang(258)."'> ".on_help($z == "sql" ? "'OPTIMIZE TABLE'" : "'VACUUM OPTIMIZE'");
                    echo "<fieldset><legend>".lang(120)." <span id='selected'></span></legend><div>".($z == "sqlite" ? $kh : ($z == "pgsql" ? $kh.$qe : ($z == "sql" ? "<input type='submit' value='".lang(259)."'> ".on_help("'ANALYZE TABLE'").$qe."<input type='submit' name='check' value='".lang(260)."'> ".on_help("'CHECK TABLE'")."<input type='submit' name='repair' value='".lang(261)."'> ".on_help("'REPAIR TABLE'") : "")))."<input type='submit' name='truncate' value='".lang(262)."'> ".on_help($z == "sqlite" ? "'DELETE'" : "'TRUNCATE".($z == "pgsql" ? "'" : " TABLE'")).confirm()."<input type='submit' name='drop' value='".lang(121)."'>".on_help("'DROP TABLE'").confirm()."\n";
                    $i = (support("scheme") ? $b->schemas() : $b->databases());
                    if (count($i) != 1 && $z != "sqlite") {
                        $j = (isset($_POST["target"]) ? $_POST["target"] : (support("scheme") ? $_GET["ns"] : DB));
                        echo "<p>".lang(263).": ", ($i ? html_select("target", $i,
                            $j) : '<input name="target" value="'.h($j).'" autocapitalize="off">'), " <input type='submit' name='move' value='".lang(264)."'>", (support("copy") ? " <input type='submit' name='copy' value='".lang(265)."'> ".checkbox("overwrite",
                                1, $_POST["overwrite"], lang(266)) : ""), "\n";
                    }
