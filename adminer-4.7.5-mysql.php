...
$pd = array(
    ...
    'sl' => 'Slovenski', 'sr' => 'Српски', 'sv' => 'Svenska', 'ta' => 'த‌மிழ்', 'th' => 'ภาษาไทย', 'tr' => 'Türkçe',
    'uk' => 'Українська', 'vi' => 'Tiếng Việt', 'zh' => '简体中文', 'zh-tw' => '繁體中文',
);
function
get_lang()
{
...
if ($_SESSION["translations_version"] != 1112001675) {
    $Og = array();
...
if (!defined("DRIVER")) {
    ...
    routine(
        ...
        foreach ($Dd
                 as $Fe) {
            $n[] = array(
                "field" => str_replace("``", "`", $Fe[2]).$Fe[3], "type" => strtolower($Fe[5]),
...
', script_src(preg_replace("~\\?.*~", "", ME)."?file=functions.js&version=4.7.5");
    if ($b->head()) {
        echo '<link rel="shortcut icon" type="image/x-icon" href="', h(preg_replace("~\\?.*~", "",
                ME)."?file=favicon.ico&version=4.7.5"), '">
...
edit_type(
    ...
    echo
    optionlist(array_merge($lc, $gg), $U), '</select>', on_help("getTarget(event).value",
...
<td><input type="radio" name="auto_increment_col" value=""><acronym id="label-ai" title="', lang(47), '">AI</acronym>', doc_link(array(
    ...
    echo '<td>', "<input type='image' class='icon' name='add[".(support("move_col") ? 0 : count($n))."]' src='".h(preg_replace("~\\?.*~",
                "",
                ME)."?file=plus.gif&version=4.7.5")."' alt='+' title='".lang(102)."'>".script("row_count = ".count($n).";"), '</thead>
<tbody>
...
', ($U == "PROCEDURE" ? "<td>".html_select("fields[$u][inout]", explode("|", $Xc), $m["inout"]) : ""), '<th>';
        ...
        echo "<td>", (support("move_col") ? "<input type='image' class='icon' name='add[$u]' src='".h(preg_replace("~\\?.*~",
                    "",
                    ME)."?file=plus.gif&version=4.7.5")."' alt='+' title='".lang(102)."'> "."<input type='image' class='icon' name='up[$u]' src='".h(preg_replace("~\\?.*~",
                    "",
                    ME)."?file=up.gif&version=4.7.5")."' alt='↑' title='".lang(103)."'> "."<input type='image' class='icon' name='down[$u]' src='".h(preg_replace("~\\?.*~",
                    "",
                    ME)."?file=down.gif&version=4.7.5")."' alt='↓' title='".lang(104)."'> " : ""), ($xe == "" || support("drop_col") ? "<input type='image' class='icon' name='drop_col[$u]' src='".h(preg_replace("~\\?.*~",
                    "", ME)."?file=cross.gif&version=4.7.5")."' alt='x' title='".lang(105)."'>" : "");
    }
}
            ...
            foreach ($rf
                ...
                foreach ($d
                    ...
                    echo "\n<div class='references' title='".h($wg)."' id='refd$sd-".($u++)."' style='left: $td"."em; top: ".$Q["fields"][$vg]["pos"]."em; height: 1.25em; background: url(".h(preg_replace("~\\?.*~",
                                "",
                                ME)."?file=arrow.gif) no-repeat right center;&version=4.7.5")."'><div style='height: .5em; border-bottom: 1px solid Gray; width: ".(-$td)."em;'></div></div>";
                }
            }
        }
        echo "\n</div>\n";
...
<th id="label-type">', lang(174), '<th><input type="submit" class="wayoff">', lang(175), '<th id="label-name">', lang(176), '<th><noscript>', "<input type='image' class='icon' name='add[0]' src='".h(preg_replace("~\\?.*~",
                "", ME)."?file=plus.gif&version=4.7.5")."' alt='+' title='".lang(102)."'>", '</noscript>
</thead>
';
    if ($Ze) {
    ...
    foreach ($L["indexes"] as $x) {
        if (!$_POST["drop_col"] || $hd != key($_POST["drop_col"])) {
            ...
            echo "<td><input name='indexes[$hd][name]' value='".h($x["name"])."' autocapitalize='off' aria-labelledby='label-name'>\n", "<td><input type='image' class='icon' name='drop_col[$hd]' src='".h(preg_replace("~\\?.*~",
                        "",
                        ME)."?file=cross.gif&version=4.7.5")."' alt='x' title='".lang(105)."'>".script("qsl('input').onclick = partial(editingRemoveRow, 'indexes\$1[type]');");
        }
        $hd++;
    }
    echo '</table>
...
';
    if (DB != "") {
        ...
        echo "<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~", "",
                    ME)."?file=plus.gif&version=4.7.5")."' alt='+' title='".lang(102)."'>\n";
    }
    echo '<input type="hidden" name="token" value="', $T, '">
</form>
';
                    ...
                    foreach ($L
                        ...
                        if (isset($Ud[$_])) {
                            ...
                            if (($_GET["modify"] && $Pb) || $Y !== null) {
                                $Hc = h($Y !== null ? $Y : $L[$_]);
                                echo ">".($zg ? "<textarea name='$v' cols='30' rows='".(substr_count($L[$_],
                                                "\n") + 1)."'>$Hc</textarea>" : "<input name='$v' value='$Hc' size='$xd[$_]'>");
                            } else {
                                $Ad = strpos($X, "<i>…</i>");
                                echo " data-text='".($Ad ? 2 : ($zg ? 1 : 0))."'".($Pb ? "" : " data-warning='".h(lang(239))."'").">$X</td>";
                            }
                        }
                    }
                    if ($Ca) {
