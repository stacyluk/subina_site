...
class phpMorphy_Dict_Writer_Sql_Schema_Mysql extends phpMorphy_Dict_Writer_Sql_Schema
{
    function dropIndex()
    {
        ...
        foreach ($indices as $key_name => $data) {
            ...
            $return_bundle->append($this->getCreateIndexStatement($key_name, $data['columns'], $data['type'],
                $data['unique']));
        }
        ...
        try {
            ...
            if (!$return_bundle->safeExecute()) {
                throw new phpMorphy_Dict_Writer_Sql_Exception(
                    "An error occured while restore old state of mysql variables: ".implode(', ',
                        $return_bundle->getLastErrors())
                );
            }
