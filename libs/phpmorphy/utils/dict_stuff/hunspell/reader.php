...
abstract class phpMorphy_Hunspell_Affix
{
    ...
    function getRemoveLength()
    {
        return $this->remove_len;
    }

    function isMorphDescription()
    {
        return isset($this->morph);
    }
...
class phpMorphy_Hunspell_DictFile
{
    ...
    function export($callback)
    {
        ...
        while ($reader->valid()) {
            ...
            if (false === call_user_func($callback, $result['word'], $all_words['lemma'], $all_words['words'],
                    $all_words['morphs'])) {
                break;
            }
        }
