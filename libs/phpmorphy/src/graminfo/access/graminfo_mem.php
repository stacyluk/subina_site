...
class phpMorphy_Graminfo_Mem extends phpMorphy_Graminfo
{
    ...
    function readFlexiaData($info)
    {
        ...
        return explode($this->ends,
            $GLOBALS['__phpmorphy_substr']($__mem, $offset, $info['affixes_size'] - $this->ends_size));
    }
    ...
    function readAllFlexia()
    {
        ...
        foreach ($this->readSectionIndexAsSize($this->header['flex_index_offset'], $this->header['flex_count'],
            $this->header['flex_size']) as $size) {
    ...
    function readAllPartOfSpeech()
    {
        ...
        foreach ($this->readSectionIndexAsSize($this->header['poses_index_offset'], $this->header['poses_count'],
            $this->header['poses_size']) as $size) {
    ...
    function readAllGrammems()
    {
        ...
        foreach ($this->readSectionIndexAsSize($this->header['grammems_index_offset'], $this->header['grammems_count'],
            $this->header['grammems_size']) as $size) {
        ...
        for ($i = 0; $i < $this->header['ancodes_count']; $i++) {
            ...
            $result[$res['id']] = array(
                ...
                'grammem_ids' => $grammems_count ?
                    array_values(unpack("v$grammems_count",
                        $GLOBALS['__phpmorphy_substr']($__mem, $offset, $grammems_count * 2))) :
                    array(),
                'offset' => $offset,
            );
