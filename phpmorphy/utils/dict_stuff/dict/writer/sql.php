...
class phpMorphy_Dict_Writer_Sql extends phpMorphy_Dict_Writer_Base
{
        ...
        try {
            ...
            /* finally */ {
                ...
                if (!$restore_keys_stmt->safeExecute()) {
                    $message = "An error occured while restore keys: ".implode(', ',
                            $restore_keys_stmt->getLastErrors());
    ...
    protected function loadLemmas(phpMorphy_Dict_Source_Normalized_Ancodes $source, $context, $dictId)
    {
        ...
        $inserter = $this->engine->getBulkInserter('lemmas',
            array('dict_id', 'base_str', 'flexia_id', 'accent_id', 'prefix_id', 'common_ancode_id'));
