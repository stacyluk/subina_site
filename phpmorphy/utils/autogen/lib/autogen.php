...
class Helper_Base
{
    ...
    function name()
    {
        return strtolower(str_replace('Helper_', '', get_class($this)));
    }
...
class StorageHelper_File extends StorageHelper
{
    function prolog()
    {
        return '$__fh = $this->resource';
    }

    function seek($offset)
    {
        return 'fseek($__fh, '.$offset.')';
    }

    function read($offset, $len)
    {
        return "fread(\$__fh, $len)";
    }
}
...
class StorageHelper_Shm extends StorageHelper
{
    function prolog()
    {
        return '$__shm = $this->resource[\'shm_id\']; $__offset = $this->resource[\'offset\']';
    }
    ...
    function read($offset, $len)
    {
        return "shmop_read(\$__shm, \$__offset + ($offset), $len)";
    }
}
...
class StorageHelper_Mem extends StorageHelper
{
    ...
    function read($offset, $len)
    {
        return "\$GLOBALS['__phpmorphy_substr'](\$__mem, $offset, $len)";
    }
}
